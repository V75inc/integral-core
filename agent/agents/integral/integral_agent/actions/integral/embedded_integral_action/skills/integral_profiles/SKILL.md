...
name: integral_profiles
description: "Inspects, authors, and modifies Integral Content Profiles — the schema layer defining a track or app's EntryTypes, Tags, and Views. Use when the user asks about profile structure, draft/publish lifecycle, or library merges."
spec: jv
allowed-tools:
...
library Content Profile that other tracks can pick up.

## When to use

The user wants to **inspect, attach, merge, or reshape** a track or app's
Content Profile — its EntryTypes, Tags, Views, or library packages. Typical
asks:
...
- Never invent `profile_template_id`, `draft_id`, `entry_type_id`,
  `view_id`, or `tag_id` values. If a needed id is missing,
  list-and-confirm first (read it via `integral_list_profiles` /
  `integral_describe_profile` / `integral_get_profile_draft`).
- Always read `integral_describe_substrate` before authoring or modifying
  ...
  materialize.
- Always read `integral_describe_profile` (or `integral_get_profile_draft`)
...
- `integral_recommend_customizations` — suggested profile tweaks for a
  ...
  `modify`/`author` proposal; still inspect with `describe_profile` and
...
- **Apply an existing library profile.** Almost always preferable to
  authoring something new. Call `integral_apply_profile_to_track` with
  `profile_template_id` (the library package) and `track_id` (the
...
- **Modify an attached profile (one discrete change).** For a single
  ...
  one EntryType, View, or Tag. Call `integral_modify_profile` with
...
- **Revise a draft precisely (batch).** When you want explicit control
  ...
  `integral_propose_profile_revision` with `draft_id` and an
...
- **Discard a draft.** Call `integral_discard_profile_draft` with
...
2. List library profiles relevant to the user's description with
   `integral_list_profiles`, and `integral_describe_profile` the target
   to see what's already attached.
3. If a good fit exists → propose `integral_apply_profile_to_track` for
   the target track.
4. If a fit exists but needs tweaks → propose
   `integral_apply_profile_to_track` first, then layer each tweak on:
   `integral_modify_profile` for a discrete add/remove of one
...
5. **Before synthesizing anything new, re-check delegation.** If there is
   ...
   - **If the request was underspecified** (no concrete entry types/
     ...
     `integral_author_profile` / `integral_draft_new_profile`.
   Once attached, iterate the draft, `integral_diff_profile_draft` to
   confirm impact, and `integral_publish_profile_draft` to go live.
...
- Proposing a field type or view type **not** confirmed by
  `integral_describe_substrate`.
- Inventing `profile_template_id`, `draft_id`, `entry_type_id`,
...
- Publishing a draft without showing the user the
  `integral_diff_profile_draft` impact first.
- Authoring a brand-new library package when an existing one fits —
  prefer `integral_apply_profile_to_track` first.
- Synthesizing a brand-new library package (`integral_author_profile`)
  or starting an empty draft (`integral_draft_new_profile`) before
...
2. `integral_describe_profile(track_id=<deals>)` → read the attached
   profile's `content_profile_id`, existing entry types, and any draft
   state.
3. `integral_get_profile_draft(content_profile_id=<from step 2>)` →
   obtain `draft_id` for the precise-revision lifecycle.
4. `integral_propose_profile_revision(draft_id=<draft_id>, operations=[
...
5. After the user blesses the revision, `integral_diff_profile_draft(
   draft_id=<draft_id>)` → explain structural changes and how many
   existing deals are affected.
6. `integral_publish_profile_draft(draft_id=<draft_id>)` → stage the
...
calling `integral_author_profile` directly:
...
Only reach for `integral_author_profile`/`integral_draft_new_profile` in
