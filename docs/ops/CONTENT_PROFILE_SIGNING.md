# Content Profile Bundle Signing — Operator Runbook

## When to use

Bundles under `backend/app/profiles/` that ship `skills/*/scripts/*.py`
run **in-process** with the integral backend. To prevent unauthorized
code from being loaded in production, set `INTEGRAL_PROFILE_PUBKEY` and
...
```bash
export INTEGRAL_PROFILE_PUBKEY=<base64-public-key>
```
...
Run: `python tools/sign_bundle.py backend/app/profiles/my-bundle /path/to/sk.b64`

## Verify

`POST /api/admin/profiles` — every loaded bundle's row shows
...
1. Land code with `INTEGRAL_PROFILE_PUBKEY` unset (dev mode — all bundles load).
2. Sign every bundle in `backend/app/profiles/` that ships python.
3. Set `INTEGRAL_PROFILE_PUBKEY` env var; restart the process.
4. Verify via `GET /api/admin/profiles` that every bundle is `signature_verified: true`.

## Rotation

To rotate keys: generate a new keypair, re-sign every bundle, then
update `INTEGRAL_PROFILE_PUBKEY` and restart. Process restart is
