# ContentProfile Extension System

## Vision: Integral as a platform

**ContentProfile** is the cornerstone of Integral's evolution from a productivity tool into a **modular, agent-authorable application platform**. Rather than building fixed features, Integral provides a runtime where **ContentProfiles define entire application experiences** that conform to any business process.

### Core Insight

A **Space** is not just a collection of Tracks—it's an **application** when powered by a ContentProfile. The CRM example demonstrates this:

```
CRM ContentProfile (scope: space)
...
## ContentProfile as Extension Mechanism

### Implementation reference

| Area | Primary code |
|------|----------------|
| Manifest compile/validate | `backend/app/services/content_profile_runtime.py` (`compile_canonical_manifest`, `VALID_FIELD_TYPES`, `VALID_VIEW_TYPES`) |
| Merge, dependencies, migrations bookkeeping | `backend/app/services/content_profile_merge.py` |
| Library CRUD, validate endpoint | `backend/app/api/content_profiles.py` |
...
|------------|--------|-------|
| Attached profiles (Space/Track) | ✅ | `HAS_CONTENT_PROFILE` edge, one per entity; `attached_content_profile_id` on Space/Track |
| Library packages | ✅ | `ContentProfiles` registry, `CATALOGS` edge; `library_package` flag filters list |
| Canonical manifest v1 | ✅ | `scope: track` or `scope: space`, validated in `compile_canonical_manifest` |
| Merge into attached profile | ✅ | `merge_library_manifest_into_content_profile` in `content_profile_merge.py` |
...
| Workspace-scoped library packages | ✅ | `POST/PUT/DELETE /api/content-profiles`, `GET /api/workspaces/{id}/content-profiles` (`content_profiles.py`) |
| Validate manifest (agents) | ✅ | `POST /api/content-profiles/validate` returns `canonical_manifest` or `issues[]` |
| Space library preview / apply | ✅ | `POST /api/spaces/{id}/content-profile/preview` and `.../apply` (preview summary + merge + `provision_prescribed_tracks`) |
| Applied migration audit | ✅ | `content_profile_migrations.run_publish_migrations` executes declarative ops on publish; `_append_applied_migrations` records the audit |
| Track templates | ✅ | `DEFINES_TRACK_PROFILE` children; `.../content-profile/track-templates` and apply-to-track APIs on spaces |
...
| Field-type registry (backend + frontend) | ✅ | `content_profile_field_types` (backend) + `frontend/src/components/entries/fieldTypes/registry.ts` (frontend); both support manifest-declared composites |
| View-type registry (backend) | ✅ | `app.views.content_profile_view_types`: palette builtins + composable meta-widgets (`composable_list/grid/board/timeline`) |
...
| Draft / publish lifecycle | ✅ | `ContentProfile.status`, `published_at`, `draft_of_id`, `version_number`; `POST /content-profiles/{id}/draft|publish|diff|discard-draft` (`content_profile_atomic_swap`) |
...
| Code plugin discovery (signed) | ✅ | `content_profile_plugins`: entry-points + directory scan; Ed25519 verify against `INTEGRAL_PLUGIN_PUBKEY`; permissive in dev |
| Substrate introspection (agent + UI) | ✅ | `GET /content-profile-substrate` returns field types, view types, plugins, registry versions |
| Agent contract (introspection-first) | ✅ | New MCP tools: `integral_describe_substrate`, `integral_describe_profile`, `integral_get_profile_draft`, `integral_propose_profile_revision`, `integral_diff_profile_draft`, `integral_publish_profile_draft`, `integral_discard_profile_draft` |
...
| Staged entry index | Partial | `_cp_index` on entries from profile `index: true` hints (`content_profile_runtime.build_entry_index_document`) |
...
| **Profile sandbox/testing** | ✅ | Draft CP + dry-run apply via `POST /content-profiles/{id}/diff` (entry-impact preview) and atomic swap on publish |
...
4. AI validates manifest:
   - `POST /api/content-profiles/validate` or `compile_canonical_manifest()` → compiled manifest or `BadRequestError` / `issues[]`
   ...
   - relation check: in-track targets or `allow_cross_track` + `target_track_types`; space graph validated in `space.relations`
                    ↓
5. User previews impact (optional): `POST /api/spaces/{id}/content-profile/preview`
                    ↓
6. Apply: `POST /api/spaces/{id}/content-profile/apply` (or `merge-library`) → merge manifest, provision prescribed tracks, materialize subgraph
```
...
Agents receive **compile-time feedback** from `compile_canonical_manifest` (and `/api/content-profiles/validate`). Representative messages (exact text from `content_profile_runtime.py`):
...
| Schema version | `manifest must set content_profile_schema_version to 1` |
| Profile dependency merge | `Missing dependency '<id>' for content profile '<cp id>'` / `Circular profile dependency detected` (`content_profile_merge.py`) |
...
[docs/content-profiles/VIEW_PALETTE.md](../content-profiles/VIEW_PALETTE.md).
...
| Backend registry | `backend/app/views/content_profile_view_types.py` | Validation + substrate introspection |
...
(`backend/app/profiles/<slug>/`). Signed code plugins remain the rare
escape hatch ([docs/content-profiles/PLUGINS.md](../content-profiles/PLUGINS.md)).

## Gaps to Address

### 1. Profile Dependency System

**Done today:** `package.dependencies[]` is part of the canonical manifest (`id` + `version` required per entry). On library merge, `_resolve_library_dependency_chain` merges dependency packages **before** the primary package (same target attached profile), with cycle detection and “missing dependency” errors. Dependency `id` is resolved against another library `ContentProfile` by node id or by matching `package.id` / `package.name` on cataloged packages.
...
**Implemented:** Workspace-scoped library packages use the same global `ContentProfiles` registry plus `workspace_id` and `library_package: true`.

| Method | Path | Role |
|--------|------|------|
| GET | `/api/content-profiles` | List cataloged library packages |
| POST | `/api/content-profiles/validate` | Validate/normalize manifest |
| POST | `/api/content-profiles` | Publish (body includes `workspace_id`, manifest or `manifest_yaml`) |
| PUT | `/api/content-profiles/{id}` | Update workspace-scoped package |
| DELETE | `/api/content-profiles/{id}` | Remove package from catalog (node deleted) |
| GET | `/api/workspaces/{id}/content-profiles` | List packages for one workspace |

**Permissions:** Workspace owner, or workspace member with `can_create_spaces` or `can_create_tracks` on `IS_MEMBER_OF` (`can_publish_content_profiles_under_workspace` in `permissions.py`). There is no separate `can_publish_content_profiles` edge yet.

### 3. Profile preview before apply

**Implemented (space):** `POST /api/spaces/{id}/content-profile/preview` returns a structured `preview` (per-track counts, relations, `would_create_tracks`, etc.) without mutating the profile. `POST .../content-profile/apply` runs merge + `provision_prescribed_tracks_from_space_manifest` and echoes `preview` plus `applied` summary.
...
```python
from integral import ContentProfileBuilder

profile = (
    ContentProfileBuilder()
    .scope("space")
...
```typescript
// In contentProfileManifest.ts or telemetry.ts
function trackProfileApply(profileId: string, spaceId: string) {
  telemetryEvent({
    name: 'content_profile_applied',
...
- [ ] Validation error messages tuned for agents (`/content-profiles/validate` exists)
...
|--------|--------|-------------|
| **Profile adoption rate** | >60% of new spaces use a library profile | Backend audit on space create |
...
| **Org-private packages** | >20% of orgs publish 1+ internal package | Org-scoped `ContentProfile` count |

## Conclusion

ContentProfile is Integral's **extension mechanism**—the bridge between a fixed tool and a living platform. By making profiles:
...
The Content Profile system is now an agent-authorable substrate organised
...
1. **Declarative type system (Pillar 1)** — primitive field/view types live
   in extensible registries (`content_profile_field_types.py`,
   `app.views.content_profile_view_types` and frontend mirrors). Manifests can declare
...
| GET  | `/api/content-profile-substrate`           | Field/view/plugin catalogue |
...
| `integral_describe_profile`          | no  | Published + draft state for a track/space CP |
| `integral_get_profile_draft`         | no  | Auto-create or fetch draft |
| `integral_propose_profile_revision`  | yes | Apply patch DSL to a draft |
| `integral_diff_profile_draft`        | no  | Diff vs published parent + entry impact |
| `integral_publish_profile_draft`     | yes | Atomic swap + migrations |
| `integral_discard_profile_draft`     | yes | Discard unpublished draft |
...
Palette convention: [docs/content-profiles/VIEW_PALETTE.md](../content-profiles/VIEW_PALETTE.md).

### Migration ops (Pillar 2)

Declared in `manifest.migrations[].ops[]`. Executed by
`content_profile_migrations.run_publish_migrations` on publish.
