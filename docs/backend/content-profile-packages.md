# Content Profile Packages and Migrations

For step-by-step authoring, PATCH APIs, and how to add packages to the **App Catalog**, see [content_profile_authoring_and_library.md](content-profile-authoring-and-library.md). For the full reference on App-scoped operational sections (skills, agents, settings, seeds, permissions), see [app_bundles_v1.md](app-bundles-v1.md).

ContentProfile manifests support package metadata and forward-only migration descriptors. Manifest schema version is **v2**; v1 has been removed (integral is pre-production, no compatibility shim retained).
...
1. Change `content_profile_schema_version: 1` → `2`
...
5. Validate via `compile_canonical_manifest()` or PATCH against a test ContentProfile; the compiler returns 400 on invalid v2 manifests
