# Authoring operational models and updating the App catalog

This guide describes how to **author** OperationalModel manifests (YAML or JSON), **attach** them to a Track or App via the API, and **extend the cataloged library** of reusable App suites and Track packs.

OperationalModel is integral's technical artifact for declarative schema + operational behavior. It applies to two scopes:
...
The user-facing term is **App**; OperationalModel is what implementers see in code and manifests. This guide covers both scopes, but the App scope is the primary authoring target — most production-grade installs are Apps, not standalone Tracks.
...
For package metadata and migrations, see [operational_model_packages.md](operational-model-packages.md). For search index staging (`_cp_index`), see [operational_model_search_index.md](operational-model-search-index.md).
...
A OperationalModel holds a **canonical manifest** (`manifest` on the node) that defines, depending on scope:
...
- **Relations** between entries (including cross-track rules when allowed by an app-level Operational Model)
...
Everything else is expected to come from OperationalModel merges.

Manifests are **compiled and validated** when you PATCH a Operational Model with `manifest` or `manifest_yaml` (see [operational_model_runtime.py](../../backend/app/services/operational_model_runtime.py)).
...
If the use case requires the user to install multiple separate Tracks and manually configure their connections, it's an App. If it's truly one Track with custom schema, it's a Track-scoped Operational Model.
...
|-------|-------------|----------|
| `track` | Track's attached operational model | One Track: its own entry types, views, tags |
| `app` | App's attached operational model | Several **track types** under `app.tracks[]`, each with entry types, views, taxonomy; optional `app.relations` for cross-track rules; optional `app.skills`, `app.agents`, `app.settings_schema`, `app.seeds`, `app.permissions` for the operational layer |

Runtime resolution for a given **Track** prefers the Track's own manifest when `scope: track`. If the Track manifest is app-scoped or missing a usable tier, the system may resolve a tier from a **parent App** Operational Model (matched by Track `template_id` or title slug vs `app.tracks[].key`).
...
2. **Apply** it by updating the attached Operational Model:
   - **Track:** `PATCH /tracks/{track_id}/operational-model` with `manifest_yaml` (string) or `manifest` (object), and `scope: track` if needed.
   - **App:** `PATCH /apps/{app_id}/operational-model` with the same, and `scope: app`.
...
package:
  name: my-pack
  description: Example track-level Operational Model
track:
  entry_types:
    - key: post
      name: Post
...
Use `allow_cross_track: true` and optional `target_track_types` when an **App** Operational Model permits references across Tracks within the App.
...
Match each real **Track** to an `app.tracks[].key` using the Track's `template_id` or a slug of its title (see resolver in `operational_model_runtime.py`).
...
backend/app/packages/my_app/     # underscored sibling for hyphenated slugs
└── tools/
    ├── __init__.py
    └── example.py
```
...
When an **app-scoped** library package is applied to a **Workspace** (for example `POST /workspaces/{id}/apps` with `library_operational_model_id`, or `POST /apps/{id}/operational-model/merge-library`), the server may **create real `Track` nodes** from `app.tracks[]`:
...
**Track-level** package merges (`POST /tracks` with `library_operational_model_id`, or track merge-library) **never** run this logic; they only merge tier data into the Track's attached Operational Model.

---

## API reference (creating / updating a Operational Model)

### Track attached Operational Model

| Method | Path | Purpose |
|--------|------|---------|
| GET | `/tracks/{track_id}/operational-model` | Read attached Operational Model (includes `manifest`) |
| PATCH | `/tracks/{track_id}/operational-model` | Update `name`, `description`, `version`, **`manifest`**, **`manifest_yaml`**, **`scope`** |
...
### App attached Operational Model

| Method | Path | Purpose |
|--------|------|---------|
| GET | `/apps/{app_id}/operational-model` | Read App Operational Model |
| PATCH | `/apps/{app_id}/operational-model` | Same as Track; use `scope: app` for App manifests |

---

## Consuming a library package on a Track or App

Library packages are **OperationalModel** nodes with `library_package: true`, listed under the app **OperationalModels** registry. They are **read** via:

- `GET /operational-models` — list packages
- `GET /operational-models/{id}` — package detail + manifest

**Merge** copies manifest-driven **entry types**, **tags/taxonomy**, **views**, and (for App-scoped packages) **skills, agents, settings_schema, seeds, permissions** into the target attached Operational Model (and creates Track-scoped nodes when a Track is provided).

| Action | Endpoint |
|--------|----------|
| Merge library into **Track** Operational Model | `POST /tracks/{track_id}/operational-model/merge-library` with `library_operational_model_id` |
| Merge library into **App** Operational Model | `POST /apps/{app_id}/operational-model/merge-library` with `library_operational_model_id` |
| Merge when **creating** a Track | `POST /tracks` with `library_operational_model_id` (and existing track CP must exist) |
| Merge when **creating** an App | `POST /workspaces/{id}/apps` with `library_operational_model_id` |

Merge logic lives in [operational_model_merge.py](../../backend/app/services/operational_model_merge.py). It understands **canonical v2** manifests (`track.*` / `app.*` tiers).
...
- optional `library_operational_model_id`
- optional `app_track_template_operational_model_id` (when creating in an App)
...
- Registry node: `OperationalModels` (`OPERATIONAL_MODELS_REGISTRY_ID`)
- Each App template: a `OperationalModel` with `library_package: true` and `scope: app`
- Each Track-scoped package: a `OperationalModel` with `library_package: true` and `scope: track`
- Membership: `CATALOGS` edge from the registry to the package (see `ensure_catalog_edge` in [app_graph.py](../../backend/app/services/app_graph.py))
...
2. **Drop bundle in** `backend/app/packages/<slug>/operational-model.yaml` (canonical path). Optional skills live under `backend/app/packages/<slug>/skills/<key>/SKILL.md`.
3. **Bootstrap or rescan**:
   - bootstrap path: `ensure_integral_app_graph()` -> shared disk sync (`operational_model_library_sync.py`)
   - hot-load path: `POST /api/admin/packages/rescan`
4. The sync layer performs slug-stable upsert (`metadata.slug`) and stale-bundle reconciliation for removed folders.
5. Check diagnostics with `GET /api/admin/packages` (`issues[]`) before publishing or applying in production.

### Widget contribution guide (view palette)

Integral treats `feed` as baseline (injected when a tier has no feed view) and
all other views as **palette keys** Operational Models compose via declarative `config`.
See [VIEW_PALETTE.md](../../docs/operational-models/VIEW_PALETTE.md) for the full
convention. Summary:
...
5. **Backend registry behavior** lives in `backend/app/views/operational_model_view_types.py` (config schema + runtime validation rules).
6. **Expose** the capability in a operational model view spec (`view_type`/`type`).
7. **Optional mobile parity**: implement the same key in mobile renderer code; Operational Model manifests do not change.
8. **Merge/apply** the Operational Model so persisted `View` rows are created for Tracks that need it.
9. **Verify** widget appears in the Track UI only when enabled by saved view capabilities.

If a Operational Model references an unregistered capability, the UI renders a clear missing-widget state so developers or AI agents can add it without changing core runtime contracts.

### AI-authoring contract

For agent-authored Operational Model changes, keep outputs deterministic and compiler-friendly:
...
1. Change the `OperationalModel`'s `manifest` / `version` in the database (or via a small admin/maintenance script).
...
Use the `package` and `migrations` sections in the manifest (see [operational_model_packages.md](operational-model-packages.md)). The compiler validates that each `migrations[]` entry includes `from_version` and `to_version`.

---

## Quick checklist

For a **Track-scoped** Operational Model:

- [ ] Set `operational_model_schema_version: 2` and `scope: track`
...
- [ ] PATCH attached Operational Model with `manifest_yaml` or `manifest`
- [ ] Optional: set `index: true` on fields for `_cp_index` extraction (see search doc)

For an **App-scoped** Operational Model:

- [ ] Set `operational_model_schema_version: 2` and `scope: app`
...
- [ ] PATCH attached App Operational Model with `manifest_yaml` or `manifest`
- [ ] For reusable App templates: register under OperationalModels registry + install via `POST /workspaces/{id}/apps`
