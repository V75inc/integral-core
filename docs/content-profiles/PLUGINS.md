...
[`backend/app/services/content_profile_plugins.py`](../../backend/app/services/content_profile_plugins.py)
...
def register(*, field_type_registry, view_type_registry):
    ...
    view_type_registry.register_view_type(
        ViewTypeSpec(
            ...
            # content_profile_view_types.py; do not add to it.
...
fetches `/api/content-profile-substrate` on app boot and caches it via
...
A profile that requires a code plugin declares the dependency so missing
...
- `backend/tests/test_content_profile_plugins.py` — directory discovery,
