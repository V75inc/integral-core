...
The region system is a **content-profile plugin bundle**
...
profile spec). This page is region-system-specific detail plus the
...
**config, not code** — an app's `profile.yaml` composes them, nothing
...
`content_profile_wizard_steps.py`, mirroring the view-type registry's shape
exactly (`WizardStepKindSpec`, `register_step_kind()`, `is_known()`).

That registry lives in core (so `content_profile_compile.py` can validate
...
**Canonical example**: `payroll-app/profile.yaml`'s `pay_run` entry type —
...
2. **One registration, backend + frontend, always paired.** Backend:
   ...
   compiles into profiles but renders as `MissingWidget` in the browser.
3. **`config_schema` is mandatory, and it's the contract.** Every key the
   ...
   agent when composing a profile, and what a future palette gallery would
   render.
...
5. **Compile-through test, not just a unit test in isolation.** Add a case
   ...
   schema store). See `tests/test_content_profile_wizard_steps.py` for the
   pattern.
