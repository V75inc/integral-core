...
profile [code plugin](PLUGINS.md) that specifically adds one or more
...
`package:` block every content-profile `profile.yaml` already uses:
...
profile* uses to declare a dependency on a plugin, and has zero real
consumers in `content_profile_compile.py` today) — `pack.yaml` describes the
pack itself, not a profile's dependency on one.

## Namespacing

Every view type a pack registers is named `pack-name/view-name` (e.g.
`example-desk/desk-board`). Enforced at `register_view_type()`
(`backend/app/views/content_profile_view_types.py`) — a plugin-sourced spec
...
`content_profile_view_types.py` so existing installed profiles keep
...
is that content profile's full view registry — both views that appear as
...
- `scope: str = "track"` (default) on `ViewTypeSpec`
  (`backend/app/views/content_profile_view_types.py`) — addressable as a
  track view/tab.
...
**Enforcement.** `content_profile_compile.py` rejects, at manifest-compile
...
direction would reject every existing profile that uses one. Track-tab
