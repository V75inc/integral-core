...
[../../backend/docs/operational_model_authoring_and_library.md](../backend/operational-model-authoring-and-library.md).
...
| **View type registry** | Backend validation + config schema + substrate introspection | `backend/app/views/operational_model_view_types.py` |
...
| **Operational Model view spec** | How a Track/App **uses** a palette key | Manifest `views[]` / attached `View` nodes |

**Rule:** Operational Models reference **keys**, not file paths. Adding a view to a Operational Model manifest never imports React code — it only declares `view_type` + `config`.
...
Operational Models may declare `view_types[]` composites (e.g. `roadmap` built on
`composable_board`). These are **Operational Model-local names** layered on palette keys,
not new palette entries. Resolved at compile time via
`register_composite_view_type` in the patch DSL.

---

## How Operational Models declare views
...
4. Unknown types → `MissingWidget` (Operational Model applied, renderer not shipped).
...
Register config schema in `backend/app/views/operational_model_view_types.py`
...
### 4. Use in Operational Models

Reference `view_type: my_view` in library bundles (`backend/app/packages/<slug>/`)
or attached Operational Models. Re-scan or bootstrap syncs library catalog rows.
...
1. **Catalog** — library `OperationalModel` packages under the `OperationalModels`
   registry (`GET /api/operational-models`, workspace-scoped lists).
...
3. **Apply** — `merge-library` on App/Track or workspace app install with
   `library_operational_model_id`; backend validates every `view_type` against
...
- Warn when a Operational Model requires a `view_type` the current client has not
...
| Surface | Purpose |
|---------|---------|
| `GET /api/operational-model-substrate` | Field types, view types (incl. palette metadata), plugins |
| `POST /api/operational-models/validate` | Compile manifest; rejects unknown `view_type` |
| `POST .../operational-model/merge-library` | Apply library package views onto attached Operational Model |
| `GET /api/operational-models` | List catalog library packages |
...
|-------------|----------------|
| `app.services.operational_model_view_types` | `app.views.operational_model_view_types` |
...
- [COMPOSITES.md](COMPOSITES.md) — Operational Model-scoped composite view types
...
- [../../OPERATIONAL_MODEL.md](../platform/operational-model.md) — platform overview
