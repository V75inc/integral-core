...
an ordinary operational-model manifest before normal compilation.

```text
App Operational Model + ui_complements[]
...
| A Operational Model-local alias/configuration of one view | Manifest composite |
...
visible in the app Operational Model.
...
placement, labels, colors, visibility, and actions remain Operational Model-owned.
...
2. Declare the required view in the app or track Operational Model using a stable `key`.
...
6. Refresh/materialize the Operational Model on existing installations; YAML changes do
...
keep it in the app Operational Model or create a domain-specific widget instead.

### Agent safety rules

- Reuse an existing `_manifest_view_key`; do not create duplicate views on
  every Operational Model refresh.
...
ordinary operational-model normalization. It also preserves the declarations in
...
- Wizard steps remain registered through `operational_model_wizard_steps`.
- Recipes expand to ordinary manifest data; they cannot execute arbitrary code.
- Domain behaviour remains in app Operational Models and tools, never generic substrate code.
