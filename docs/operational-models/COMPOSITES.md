...
Operational Model manifest declare new field/view types that decorate an existing
...
specificity *in the manifest itself*, so they ship with the Operational Model and
...
`operational_model_field_types` / `operational_model_view_types`. Composites
...
`compile_canonical_manifest` (in `app/services/operational_model_runtime.py`)
...
- `backend/tests/test_operational_model_registries.py` — composite
