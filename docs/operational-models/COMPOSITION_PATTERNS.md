...
- `Contacts` track in the CRM app (`backend/app/packages/crm/`): a contact
...
- Turnkey wiring: `crm-pm-workspace` workspace Operational Model.
...
- The child container is heavyweight — mixed entry types, multiple
  views, its own OperationalModel-scoped behavior.
...
- The provisioned Track is attached **by reference** to a TEMPLATE
  OperationalModel registered under the Space's
...
- The parent Entry carries a `relation` field with `target: track`,
  ...
  `backend/app/services/operational_model_graph.py`.
- Provenance is recorded via the new `TEMPLATED_FROM` edge
  (`Track → OperationalModel`; see Resolved Fork 4 below). `USES_TEMPLATE`
...
- `Projects` track in `backend/app/packages/projects/`: each `Project` entry's
...
Auto-provisioned anchored Tracks `HAS_OPERATIONAL_MODEL` the TEMPLATE's
existing `OperationalModel` node — there is no clone, no copy, no fork.
Two `Project` entries' anchored Tracks share the same
`Track.attached_operational_model_id` scalar AND the same
`HAS_OPERATIONAL_MODEL` edge target. Per-anchor variance (fork-on-edit,
...
A NEW edge `TEMPLATED_FROM` (`Track → OperationalModel`) records template
...
space:
  track_templates:
    - { key: <slug>, name: <display>, template_slug|operational_model_id: <ref> }
```
...
`backend/app/services/operational_model_graph.py` and
`backend/app/services/operational_model_compile.py`.
...
### No per-anchor OperationalModel clone in v1
...
1. `_validate_relation_values` in `operational_model_compile.py`
   (manifest-validator layer) — rejects manifests that reference a
   cross-workspace track template id.
2. `materialize_anchor_track` in `operational_model_graph.py`
...
`backend/app/services/operational_model_graph.py`, invoked via the
...
Agents authoring or modifying a OperationalModel that wants to use the
...
5. Validate the draft via `integral_diff_model_draft` before
...
- `backend/app/packages/crm-pm-workspace/operational-model.yaml` —
