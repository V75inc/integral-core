...
| d03 | `integral_scaffold` field palette | Implied tags are part of the build. | Not in `_PLAN_TOOLS`. | Interim: tags are a post-build step (scaffold may call `integral_create_tag`). W1.1 replaces this wording when build support ships. | Fixed (interim) | d03 |
