...
| C1 — Distributable Core and SDK | **Partial** | Core-only and contract lanes, SDK source, package discovery, external reference-App materials, and developer guidance exist. `make verify-artifact` now builds the Core wheel, imports it outside the source tree, confirms required runtime registries are packaged, and excludes internal domain profiles. | Build and install the actual Core and SDK in a clean environment, boot it without source-tree coupling, and prove that an independently built App loads through the documented public contract. |
