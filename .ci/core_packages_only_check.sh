...
if [ ! -d "$PROFILES" ]; then
  echo "core-packages-only: FAIL — missing $PROFILES" >&2
  exit 1
fi
...
while IFS= read -r -d '' entry; do
  ...
  if ! echo "$name" | grep -Eq "$ALLOWED"; then
    echo "core-packages-only: unexpected under app/packages/: $name" >&2
    bad=1
  fi
...
while IFS= read -r -d '' yaml; do
  if grep -Eq 'class:[[:space:]]*(community_app|verified_app|commercial_app|private_org_app)' "$yaml"; then
    echo "core-packages-only: non-core package.class in $yaml" >&2
    bad=1
  fi
done < <(find "$PROFILES" -name operational-model.yaml -print0 2>/dev/null || true)

if [ "$bad" -ne 0 ]; then
  echo "core-packages-only: FAIL — move domain packages to packages/apps/" >&2
  exit 1
fi

echo "core-packages-only: OK (seeds only under backend/app/packages/)"
