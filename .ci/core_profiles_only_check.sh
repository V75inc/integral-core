#!/usr/bin/env bash
# F0/OSS extract guard — backend/app/profiles/ must contain only core seeds.
...
while IFS= read -r -d '' entry; do
  ...
  if ! echo "$name" | grep -Eq "$ALLOWED"; then
    echo "core-profiles-only: unexpected under app/profiles/: $name" >&2
    bad=1
  fi
done < <(find "$PROFILES" -mindepth 1 -maxdepth 1 -print0)

# Also fail if a profile.yaml under app/profiles declares non-core class.
...
done < <(find "$PROFILES" -name profile.yaml -print0 2>/dev/null || true)
...
echo "core-profiles-only: OK (seeds only under backend/app/profiles/)"
