...
async def test_template_view_without_track_id_is_denied():
    view = await View.create(
        ...
        type="kanban",
        track_id="",
        operational_model_id="cp-shared",
        is_template=True,
    )
...
async def test_shared_cp_admin_on_sibling_does_not_grant_edit_on_other_track_view():
    ...
    merely because both share the same template OperationalModel."""
    ...
    track_a = await Track.create(
        ...
        attached_operational_model_id=cp.id,
    )
    ...
    await track_a.connect(cp, edge=HAS_OPERATIONAL_MODEL)
    await track_b.connect(cp, edge=HAS_OPERATIONAL_MODEL)
