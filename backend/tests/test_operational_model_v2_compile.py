...
def test_v1_manifest_rejected_with_upgrade_message():
    """v1 manifest input raises OperationalModelV1RejectedError carrying the
    ...
    with pytest.raises(OperationalModelV1RejectedError) as excinfo:
    ...
    assert isinstance(excinfo.value, OperationalModelValidationError)
...
def test_v1_manifest_rejection_is_also_caught_by_base_error():
    ...
    raw = {"operational_model_schema_version": 1, "scope": "track"}
...
def test_v2_manifest_with_no_app_sections_compiles():
    ...
    assert out["operational_model_schema_version"] == 2
...
#
# An app.tracks[] track's own attached OperationalModel is materialized by
# taking ITS compiled output tier, stripping a few keys
# (operational_model_merge._track_spec_to_library_manifest_dict), wrapping it
# as a fresh scope: "track" manifest, and compiling THAT (operational_model_
# merge.apply_space_track_spec_to_track -> merge_library_manifest_into_
# operational_model). suppress_feed_fallback must not be a one-shot flag that
