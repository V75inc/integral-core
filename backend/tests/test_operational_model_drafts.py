...
@pytest.mark.asyncio
class TestOperationalModelDrafts:
    ...
    async def test_fork_draft_creates_sibling(
        ...
        fork = await authenticated_client.post(
            f"/api/operational-models/{published['id']}/draft", json={}
        )
    ...
    async def test_publish_swaps_manifest_and_bumps_version(
        ...
        publish = await authenticated_client.post(
            f"/api/operational-models/{draft['id']}/publish",
        ...
        re_get = await authenticated_client.get(
            f"/api/operational-models/{published_id}"
        )
        assert re_get.status_code == 200
        manifest = re_get.json()["operational_model"]["manifest"]
    ...
    async def test_discard_draft_removes_it(
        ...
        discard = await authenticated_client.post(
            f"/api/operational-models/{draft_id}/discard-draft", json={}
        )
        ...
        publish = await authenticated_client.post(
            f"/api/operational-models/{draft_id}/publish", json={}
        )
    ...
    async def test_substrate_endpoint_returns_registries(
        ...
        resp = await authenticated_client.get("/api/operational-model-substrate")
