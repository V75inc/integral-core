"""TEST-01 gap closure for POST /api/content-profiles/{id}/preview-update (Plan 07-05).
...
async def test_preview_update_invalid_sample_limit_raises_422(
    ...
    res = await authenticated_client.post(
        "/api/content-profiles/some-id/preview-update?sample_limit=abc",
    )
...
async def test_preview_update_nonexistent_id_returns_404_or_403(
    ...
    """Bogus content_profile_id → 404 (ResourceNotFound) or 403 (policy gate).
    ...
    """
    res = await authenticated_client.post(
        "/api/content-profiles/no-such-cp/preview-update?sample_limit=10",
    )
    assert res.status_code == 422, res.text
