...
from app.api.content_profiles import (
...
async def test_publish_single_yaml_via_json_body(authenticated_client: AsyncClient):
    """JSON body publish path creates a library ContentProfile node."""
    # Seed the ContentProfiles registry (test DB starts empty; startup seeder
    ...
    if not reg:
        await ContentProfiles.create(id=CONTENT_PROFILES_REGISTRY_ID)
...
def test_zip_one_profile_extraction():
    """ZIP with one profile.yaml: extraction yields one manifest dict."""
    import pathlib
    import tempfile

    zip_bytes = _make_zip({"my-pkg/profile.yaml": _MINIMAL_YAML})
...
def test_zip_two_profiles_extraction():
    """ZIP with two profile.yaml files: extraction yields two manifest dicts."""
    import pathlib
    import tempfile

    zip_bytes = _make_zip(
        {
            "first-pkg/profile.yaml": _MINIMAL_YAML,
            "second-pkg/profile.yaml": _SECOND_YAML,
        }
    )
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = pathlib.Path(tmp)
...
def test_zip_no_profiles_raises_bad_request():
    """ZIP with no profile.yaml files: extraction finds nothing, BadRequestError expected."""
    import pathlib
    import tempfile
...
def test_zip_nested_files_not_included():
    """Files deeper than one level (e.g. pkg/sub/profile.yaml) are not picked up."""
    import pathlib
    import tempfile

    zip_bytes = _make_zip(
        {
            "pkg/sub/profile.yaml": _MINIMAL_YAML,  # two levels deep — should be ignored
            "pkg-root/profile.yaml": _SECOND_YAML,  # one level deep — should be included
        }
    )
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = pathlib.Path(tmp)
        ...
        with zipfile.ZipFile(zip_path) as zf:
            zf.extractall(tmp_path)
        # glob("*/profile.yaml") only matches one-level deep
        profile_files = sorted(tmp_path.glob("*/profile.yaml"))
        assert len(profile_files) == 1
...
def test_compile_canonical_manifest_from_yaml_string():
    """compile_canonical_manifest accepts a yaml.safe_load() result from a profile.yaml."""
