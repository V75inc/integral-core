...
``POST /api/operational-models/{id}/preview-update`` alias (MIG-04) and the
...
async def _create_published_library_cp(
    ...
    """Mirror the helper in test_operational_model_drafts.py.
...
async def _fork_and_mutate_draft(
    ...
    *,
    ...
    fork = await authenticated_client.post(
        f"/api/operational-models/{published_id}/draft", json={}
    )
    ...
    upd = await authenticated_client.put(
        f"/api/operational-models/{draft['id']}",
        json={"manifest": new_manifest},
    )
    assert upd.status_code == 200, upd.text
    return upd.json()["operational_model"]


@pytest.mark.asyncio
class TestPreviewUpdateAlias:
    """MIG-04 — POST /api/operational-models/{id}/preview-update."""
    ...
    async def test_alias_responds_200_with_diff_payload_shape(
        ...
        resp = await authenticated_client.post(
            f"/api/operational-models/{draft['id']}/preview-update",
    ...
    async def test_alias_and_diff_return_identical_payloads(
        ...
        diff_resp = await authenticated_client.post(
            f"/api/operational-models/{draft['id']}/diff",
            json={"include_entry_impact": False},
        )
    ...
    async def test_404_when_cp_not_found(
        ...
        """Test 12: bogus operational_model_id → 404."""
        resp = await authenticated_client.post(
            "/api/operational-models/cp:does_not_exist/preview-update",
            json={},
        )
...
class TestPreviewUpdatePayloadRollup:
    async def test_would_need_migration_true_when_any_impact_needs_migration(
        ...
        from app.services import operational_model_diff as diff_mod
...
async def test_unauthenticated_preview_update_rejected(client: AsyncClient):
    ...
    resp = await client.post(
        "/api/operational-models/cp:anything/preview-update", json={}
    )
