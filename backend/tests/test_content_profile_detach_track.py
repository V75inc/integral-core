"""Plan 07-02 — Coverage for POST /api/tracks/{track_id}/content-profile/detach-library.

Locks the existing track-side detach endpoint at
``backend/app/api/content_profiles.py:665``. Plan 07-02 does NOT change
...
class TestDetachLibraryFromTrack:
    """LIB-03: POST /api/tracks/{track_id}/content-profile/detach-library."""
    ...
    async def test_happy_path_drops_library_merge_source_id(
        ...
        resp = await authenticated_client.post(
            f"/api/tracks/{tid}/content-profile/detach-library", json={}
        )
    ...
    async def test_detach_then_remerge_does_not_duplicate_entry_types(
        ...
        cp1 = await authenticated_client.get(f"/api/tracks/{tid}/content-profile")
        assert cp1.status_code == 200
        keys_before = sorted(
            et["key"]
            for et in (
                (cp1.json().get("content_profile") or {})
                .get("manifest", {})
                .get("track", {})
                .get("entry_types", [])
            )
        ...
        cp2 = await authenticated_client.get(f"/api/tracks/{tid}/content-profile")
        assert cp2.status_code == 200
        ets_after = (
            (cp2.json().get("content_profile") or {})
            .get("manifest", {})
            .get("track", {})
            .get("entry_types", [])
        )
