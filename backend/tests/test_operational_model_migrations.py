...
async def test_populated_field_migration_preserves_record_graph_and_view_bindings():
    ...
    with (
        ...
        patch(
            "app.services.operational_model_migrations._affected_tracks",
...
def _patch_entry_type_get(et_by_id):
    ...
    return patch(
        "app.services.operational_model_migrations.EntryType.get",
        side_effect=_get,
    )
...
async def test_run_publish_migrations_dispatches_new_ops():
    ...
    with (
        patch.dict(
            "app.services.operational_model_migrations._OP_HANDLERS",
            {
