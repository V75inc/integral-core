"""Regression: a draft ContentProfile must resolve edit rights via its
published parent.

A draft CP carries the parent's scope (``track``/``app``) but is attached to
no Track/App — only the published parent holds the
``attached_content_profile_id`` back-pointer. Gating edit permission on the
draft node's own id alone found no owner and denied a user who plainly owns the
underlying track — the bug that made ``integral_get_profile_draft`` return
...
from app.services import agent_profiles
...
async def test_draft_cp_edit_resolves_via_published_parent(monkeypatch):
    ...
    draft = _CP("n.ContentProfile.draft", "track", draft_of_id=parent_id)
    assert await agent_profiles._user_can_edit_cp(user_id="u1", cp=draft) is True
...
async def test_orphan_cp_without_parent_is_denied(monkeypatch):
    ...
    orphan = _CP("n.ContentProfile.orphan", "track", draft_of_id=None)
    assert await agent_profiles._user_can_edit_cp(user_id="u1", cp=orphan) is False
