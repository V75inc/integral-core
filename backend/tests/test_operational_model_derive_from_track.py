"""Plan 07-02 — Coverage for POST /api/operational-models/from-track/{track_id}.
...
class TestDeriveLibraryFromTrack:
    """LIB-02 + ROADMAP AC#2: POST /api/operational-models/from-track/{track_id}."""
    ...
    async def test_from_track_stamps_provenance(
        ...
        resp = await authenticated_client.post(
            f"/api/operational-models/from-track/{tid}",
            json={"name": "Derived Pack"},
        )
        assert resp.status_code == 200, resp.text
        body = resp.json()
        derived = body["operational_model"]
    ...
    async def test_from_track_404_when_track_id_unknown(
        ...
        resp = await authenticated_client.post(
            "/api/operational-models/from-track/cp-track-doesnotexist",
            json={"name": "Phantom"},
        )
    ...
    async def test_from_track_derived_package_round_trips(
        ...
        derived = derive.json()["operational_model"]
        ...
        merge = await authenticated_client.post(
            f"/api/tracks/{new_tid}/operational-model/merge-library",
            json={"library_operational_model_id": derived_id},
        )
        assert merge.status_code == 200, merge.text

        cp_resp = await authenticated_client.get(
            f"/api/tracks/{new_tid}/operational-model"
        )
        assert cp_resp.status_code == 200, cp_resp.text
        manifest = (cp_resp.json().get("operational_model") or {}).get("manifest") or {}
