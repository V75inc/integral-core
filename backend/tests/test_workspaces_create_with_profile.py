"""Tests for ``POST /workspaces?operational_model_slug=...`` and the new
``GET /library/workspace-models`` listing endpoint (Phase D4).

The first three cases exercise the strict-init wiring on workspace create:

  - Regression: ``operational_model_slug`` omitted → existing flow unchanged, no
    ``applied_profiles`` entry recorded.
  - ``operational_model_slug`` referencing a non-workspace-scope bundle → 4xx +
    workspace rolled back (so the caller doesn't end up with an empty,
    half-provisioned org).
  - ``operational_model_slug`` referencing an unknown bundle → 404 + workspace
    rolled back.

The fourth case covers ``GET /library/workspace-models`` — at the time
...
async def test_create_blank_workspace_still_works(authenticated_client):
    """Regression — workspaces without ``operational_model_slug`` still work.
...
async def test_create_workspace_with_non_workspace_scope_profile_rolls_back(
    ...
    r = await authenticated_client.post(
        ...
        json={
            "name": "Bad Scope D4",
            "operational_model_slug": "personal-crm",  # scope=app, not workspace
        },
    )
...
async def test_create_workspace_with_unknown_profile_returns_404(
    ...
    r = await authenticated_client.post(
        ...
        json={
            ...
            "operational_model_slug": "does-not-exist-d4",
        },
    )
...
@pytest.mark.asyncio
async def test_list_workspace_operational_models_returns_only_workspace_scope(
    authenticated_client,
):
    """Workspace-profile listing excludes track/app-scope bundles.

    The endpoint filters ``load_library_operational_models()`` by
    ...
    """
    r = await authenticated_client.get("/api/library/workspace-models")
    assert r.status_code == 200, r.text
    profiles = r.json().get("operational_models", [])
