"""TEST-01 gap closure for /api/content-profiles/author + /{id}/modify (Plan 07-05).
...
@pytest.mark.asyncio
async def test_author_profile_missing_required_fields_returns_422_or_400(
    authenticated_client: AsyncClient,
) -> None:
    """POST /api/content-profiles/author with empty body.
...
@pytest.mark.asyncio
async def test_author_profile_invalid_type_hint_raises_422(
    authenticated_client: AsyncClient,
) -> None:
...
@pytest.mark.asyncio
async def test_modify_profile_missing_body_returns_422_or_400(
    authenticated_client: AsyncClient,
) -> None:
    """POST /api/content-profiles/{id}/modify with empty body."""
    resp = await authenticated_client.post(
        "/api/content-profiles/some-id/modify",
        json={},
    )
...
@pytest.mark.asyncio
async def test_modify_profile_invalid_patches_shape_raises_422(
    authenticated_client: AsyncClient,
) -> None:
