"""Unit tests for operational model merge helpers (library manifests)."""
...
class _FakeLibraryCP:
    """Minimal stand-in for OperationalModel that the merge treats as a library."""
...
async def test_space_merge_propagates_track_templates():
    ...
    lib = _FakeLibraryCP(lib_manifest, "n.OperationalModel.lib1")
    target = _FakeTargetCP(target_manifest, "n.OperationalModel.target1")
...
async def test_space_merge_track_templates_dedupes_by_key():
    ...
    lib = _FakeLibraryCP(lib_manifest, "n.OperationalModel.lib2")
    target = _FakeTargetCP(target_manifest, "n.OperationalModel.target2")
