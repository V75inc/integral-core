"""Plan 07-02 — Coverage for POST /api/apps/{app_id}/operational-model/detach-library.

Mirrors the existing track-side detach endpoint at
``backend/app/api/operational_models.py:665`` for the App-attached
OperationalModel, locking I-LIB-04 (non-cascading: DEFINES_TRACK_PROFILE
edges to track-template OperationalModels are preserved).
"""
...
async def _create_space_with_library(
    ...
    sp = await authenticated_client.post(
        "/api/apps",
        json={"name": "Detach-Sp App", "library_operational_model_id": library_id},
    )
...
class TestDetachLibraryFromApp:
    """LIB-03: POST /api/apps/{app_id}/operational-model/detach-library."""
    ...
    async def test_happy_path_detaches_library_provenance(
        ...
        resp = await authenticated_client.post(
            f"/api/apps/{app_id}/operational-model/detach-library", json={}
        )
    ...
    async def test_404_when_space_id_unknown(
        ...
        resp = await authenticated_client.post(
            "/api/apps/cp-doesnotexist/operational-model/detach-library", json={}
        )
        assert resp.status_code in (403, 404), resp.text
    ...
    async def test_defines_track_profile_edges_preserved_after_detach(
        ...
        DEFINES_TRACK_PROFILE. Materialized track-template OperationalModel rows
