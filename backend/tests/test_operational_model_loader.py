"""Tests for the YAML-based library Operational Model loader."""
...
def write_profile(root: Path, slug: str, data: dict) -> None:
    ...
    (pkg_dir / "operational-model.yaml").write_text(yaml.dump(data), encoding="utf-8")


def test_load_empty_directory():
    with tempfile.TemporaryDirectory() as tmp:
        specs = load_library_operational_models(Path(tmp))
    assert specs == []
...
def test_load_single_track_profile():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_profile(
            ...
            "bug-tracker",
            {
                "integral_operational_model_version": 2,
                "scope": "track",
                "package": {
                    "name": "Bug Tracker",
                    "slug": "bug-tracker",
        ...
        specs = load_library_operational_models(root)
    ...
    assert s.manifest["operational_model_schema_version"] == 2
...
def test_load_skips_malformed_yaml(caplog):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        bad_dir.mkdir()
        (bad_dir / "operational-model.yaml").write_text(
...
def test_load_production_profiles():
    """Smoke test: all profiles in app/packages/ load without error."""
    from pathlib import Path as P

    import app

    packages_root = P(app.__file__).parent / "packages"
    if not packages_root.exists():
        pytest.skip("No profiles directory found")
    specs = load_library_operational_models(packages_root)
    assert len(specs) > 0
    for s in specs:
        assert s.name
        assert s.manifest.get("operational_model_schema_version") == 2
