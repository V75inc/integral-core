"""Plan 07-02 — Coverage for POST /api/tracks/{track_id}/operational-model/revert-customizations.

Locks I-LIB-05: revert is reject-gated by
``compute_entry_impact_for_attached``. Track-side endpoint at
``backend/app/api/operational_models.py:745``.
"""
...
class TestRevertTrackProfileCustomizations:
    """LIB-03 + ROADMAP AC#3: POST /api/tracks/{track_id}/operational-model/revert-customizations."""
    ...
    async def test_happy_path_revert_with_no_entries(
        ...
        resp = await authenticated_client.post(
            f"/api/tracks/{tid}/operational-model/revert-customizations",
            json={},
        )
    ...
    async def test_400_when_track_has_no_library_provenance(
        ...
        tid = tr.json()["track"]["id"]
        resp = await authenticated_client.post(
            f"/api/tracks/{tid}/operational-model/revert-customizations", json={}
        )
    ...
    async def test_revert_emits_change_event_with_revert_details(
        ...
        """D-05 emission: the revert handler emits operational_model.update with
        ...
        # operational_model.update event with details.revert.
        ...
        revert_events = [
            e
            for e in events
            if e.get("action") == "operational_model.update"
            and (e.get("details") or {}).get("revert") is True
        ]
        assert (
            revert_events
        ), "no operational_model.update event with details.revert=True found"
