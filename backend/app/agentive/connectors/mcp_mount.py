...
async def mount_mcp_connector(
    *,
    ...
    connector.label = (label or display_name or "").strip()
...
async def refresh_mcp_connector(connector: Any) -> Any:
    ...
    try:
        ...
        await connector.save()
        catalog_slug = catalog_slug_for_connector(connector)
        register_mcp_tools(workspace_id, connector.id, tools, catalog_slug=catalog_slug)
    ...
    for c in connectors or []:
        ...
        try:
            ...
            register_mcp_tools(
                ...
                c.id,
                tools,
                catalog_slug=catalog_slug_for_connector(c),
            )
            try:
