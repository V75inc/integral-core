"""ContentProfile helper functions exposed to the embedded agent.
...
profile-related MCP tools (``integral_author_profile``,
``integral_modify_profile``, ``integral_list_profiles``). They are
...
async def list_library_profiles(
    *,
    keyword: Optional[str] = None,
    scope: Optional[str] = None,
) -> Dict[str, Any]:
    """List library Content Profile packages with optional filters.

    Supports keyword filtering and scope filtering ('track' or 'app').

    Mirrors ``integral_list_profiles`` from the MCP tool surface.
    """
    ...
    cps_reg = await ContentProfiles.get(CONTENT_PROFILES_REGISTRY_ID)
    if not cps_reg:
        return {"profiles": [], "total": 0}

    lib_profiles = await cps_reg.nodes(edge=["CATALOGS"], node=["ContentProfile"])
...
async def get_attached_profile(
    *,
    user_id: str,
    track_id: Optional[str] = None,
    app_id: Optional[str] = None,
) -> Dict[str, Any]:
    """Describe the ContentProfile attached to a track or app_node.
    ...
    if not cp:
        return {
            ...
            "message": "No ContentProfile attached.",
        }
...
def _short_profile_name(name: Optional[str], description: str) -> str:
    """A clean, short label for a content profile / its starter entry type.
...
def _build_manifest_entry_types(
    ...
    back to the single empty starter type named after the profile (legacy shape).
    """
    from app.services.content_profile_runtime import slug_manifest_key
...
async def apply_entry_types_to_track(
    *,
    ...
    attached ContentProfile, replacing the generic starter ``Post`` type.
    ...
    the exact shape the agent already produces for ``integral_author_profile``.
...
async def author_profile(
    *,
    ...
    """Create a new library Content Profile from a description.
    ...
    Returns the new ContentProfile id, name, and the compiled manifest.
    """
    ...
    from app.services.permissions import can_publish_content_profiles_under_workspace
    ...
    if not decision.allowed:
        return {"error": "forbidden", "detail": "profile.author denied"}
    ...
    profile_name = _short_profile_name(name, description)
    ...
    return {
        ...
        "name": cp.name,
        "manifest": compiled,
        "message": f"Authored library profile '{profile_name}'",
    }
...
async def modify_profile(
    *,
    ...
    """Add or remove an EntryType / View / Tag on an attached ContentProfile.

    Operates on the profile attached to a Track or App.

    Mirrors ``integral_modify_profile`` from the MCP tool surface
    ...
    if track_id:
        ...
        if not track_obj:
            return {"error": "not_found", "detail": "Track not found"}
        cp = await get_track_attached_content_profile(track_obj)
        if not cp:
            return {
                "error": "not_found",
                "detail": "Track has no attached content profile",
            }
    else:
        app_node = await App.get(app_id)
        if not app_node:
        ...
        if not cp:
            return {
                "error": "not_found",
                "detail": "App has no attached content profile",
            }

    if not await _user_can_edit_cp(user_id=user_id, cp=cp):
        return {"error": "forbidden", "detail": "no edit access to content profile"}
    ...
    if action == "add_entry_type":
        ...
        # agent already knows from integral_author_profile —
        ...
        # working author_profile→materialize flow uses, so structured fields
...
async def apply_library_profile(
    *,
    user_id: str,
    library_profile_id: str,
    track_id: Optional[str] = None,
    app_id: Optional[str] = None,
) -> Dict[str, Any]:
    """Merge a library Content Profile package into the attached profile.

    Operates on the profile attached to a track or app_node.
    ...
    if not library_profile_id:
        return {"error": "missing_argument", "detail": "library_profile_id is required"}

    from app.schemas.policy import Resource, Subject
    from app.services.policy_engine import evaluate as policy_evaluate

    lib = await ContentProfile.get(library_profile_id)
    if not lib or not getattr(lib, "library_package", False):
        return {"error": "not_found", "detail": "Library content profile not found"}
    ...
    if track_id:
        ...
        if not attached:
            return {
                "error": "not_found",
                "detail": "Track has no attached profile to merge into",
            }
        await merge_library_manifest_into_content_profile(
            lib, attached, track, for_space=False
        )
        track.library_merge_source_id = library_profile_id
        await track.save()
        return {
            "track_id": track_id,
            ...
            "message": (
                f"Applied library profile '{getattr(lib, 'name', library_profile_id)}' "
                f"to track."
            ),
        }
    ...
    if not attached:
        return {
            "error": "not_found",
            "detail": "App has no attached profile to merge into",
        }
    await merge_library_manifest_into_content_profile(
        lib, attached, track=None, for_space=True
    )
    app_node.library_merge_source_id = library_profile_id
    await app_node.save()
    return {
        "app_id": app_id,
        "library_profile_id": library_profile_id,
...
# (``integral_describe_substrate``, ``integral_describe_profile``,
# ``integral_get_profile_draft``, ``integral_propose_profile_revision``,
# ``integral_diff_profile_draft``, ``integral_sandbox_apply_to_draft``,
# ``integral_publish_profile_draft``, ``integral_discard_profile_draft``)
...
async def describe_substrate() -> Dict[str, Any]:
    ...
    Mirrors ``GET /api/content-profile-substrate`` for in-process callers.
...
async def describe_profile(
    *,
    ...
    if cp is None:
        return {
            "error": "not_found",
            "detail": "no attached content profile",
        }

    # Find sibling draft (if any).
    drafts = await ContentProfile.find(
        {
...
async def get_or_create_draft(
    *,
    ...
    """Fetch or create a draft for ``content_profile_id``.
    ...
    Otherwise forks a new draft via ``content_profile_atomic_swap``.
    ...
    if cp is None:
        return {"error": "not_found", "detail": "content profile not found"}
...
async def apply_patch_to_draft(
    *,
    ...
    draft = await ContentProfile.get(draft_id)
    if draft is None:
...
async def publish_draft_for_agent(
    *,
    ...
    from app.exceptions import BadRequestError as _BadRequest
    from app.models.nodes import ContentProfile
    from app.services.content_profile_atomic_swap import publish_draft as _impl
...
async def discard_draft_for_agent(
    *,
    ...
    from app.models.nodes import ContentProfile
    from app.services.content_profile_atomic_swap import discard_draft as _impl
...
async def create_empty_library_draft(
    ...
    can immediately pass ``draft_id`` to ``integral_propose_profile_revision``.
    """
    from app.models.nodes import (
        CONTENT_PROFILES_REGISTRY_ID,
        ContentProfile,
...
async def apply_library_to_track(
    ...
    """Merge a published library package into a track's attached content profile.
    ...
    # Track-attached profile mutation is admin-tier substrate authority
    ...
    lib = await ContentProfile.get(library_cp_id)
    if not lib or not getattr(lib, "library_package", False):
        return {"error": "not_found", "detail": "Library content profile not found"}
    if getattr(lib, "status", None) != "published":
        return {
            "error": "bad_request",
            "detail": "Library content profile must be published before applying",
        }

    attached = await get_track_attached_content_profile(track)
    if not attached:
        return {"error": "not_found", "detail": "Track has no attached content profile"}
...
async def recommend_profile_customizations(
    ...
    """Analyze a track's entries and attached profile to surface patch suggestions.

    Returns a list of concrete patch ops (each passable directly to
    ``integral_propose_profile_revision``) plus summary statistics about the
    sample analyzed.
    """
...
async def _user_can_edit_cp(*, user_id: str, cp: Any) -> bool:
    ...
    # (only the published parent holds the ``attached_content_profile_id``
    # back-pointer; see fork_draft in content_profile_atomic_swap). Gating on
    ...
    if scope == "track":
        for cid in candidate_cp_ids:
            owners = await Track.find({"context.attached_content_profile_id": cid})
    ...
    if scope == "app":
        for cid in candidate_cp_ids:
            owners = await App.find({"context.attached_content_profile_id": cid})
