"""Canonical operational model runtime helpers.

Orchestration facade over split modules — see ``.planning/refactors/operational_model_runtime_split_plan.md``.
"""
...
from app.models.nodes import App, OperationalModel, Track, View

# Public re-exports (import paths unchanged for callers).
from app.services.operational_model_compile import (  # noqa: F401
    SCHEMA_VERSION,
...
from app.services.operational_model_entry_fields import (  # noqa: F401
...
from app.services.operational_model_graph import (  # noqa: F401
...
async def write_view_entry_type_constraints_from_manifest(
    ...
    Called from ``operational_model_merge`` after view materialization so
...
async def _try_refresh_anchor_template_cp(
    track: Track, stale_cp: OperationalModel
    ...
    try:
        ...
        if definition is not None and definition.canonical_manifest:
            ...
            canonical = compile_canonical_manifest(
                manifest=_as_dict(
                    app_cp.manifest, where="app operational model manifest"
                )
            )
    ...
    refreshed = await _resolve_or_create_template_operational_model(
        app_node=app_node,
...
async def resolve_track_runtime_profile(
    track: Track,
) -> Tuple[Optional[OperationalModel], Dict[str, Any], Optional[str]]:
    """Resolve effective profile for a track.

    Returns ``(operational_model, tier, track_type_key)`` where tier is a canonical
        ...
        try:
            manifest = compile_canonical_manifest(
                manifest=repair_stored_manifest_for_compile(
                    _as_dict(cp.manifest, where="operational model manifest")
                )
            )
        except BadRequestError:
            ...
            manifest = compile_canonical_manifest(
                manifest=repair_stored_manifest_for_compile(
                    _as_dict(refreshed.manifest, where="operational model manifest")
                )
            )
            cp = refreshed
                    ...
                    try:
                        r_manifest = compile_canonical_manifest(
                            manifest=repair_stored_manifest_for_compile(
                                _as_dict(
                                    refreshed.manifest,
                                    where="operational model manifest",
                                )
                            )
                        )
                    except BadRequestError:
    ...
    if apps:
        ...
        if isinstance(app_node, App):
            ...
            definition = await get_active_application_definition(app_node)
            scp = await get_app_attached_operational_model(app_node)
            if definition is not None and definition.canonical_manifest:
                manifest = dict(definition.canonical_manifest)
            elif scp and scp.manifest:
                manifest = compile_canonical_manifest(
                    manifest=repair_stored_manifest_for_compile(
                        _as_dict(scp.manifest, where="app operational model manifest")
                    )
                )
            else:
                manifest = {}
...
async def ensure_track_views_materialized_from_tier(track: Track) -> bool:
    ...
    Anchored tracks share a by-reference template OperationalModel. Views must be
...
class LibraryPackageSummary:
    """Typed summary of a catalogued library-package OperationalModel.
    ...
    ``GET /api/operational-models`` projection, but as a Python dataclass.
    """
...
async def list_library_packages() -> List[LibraryPackageSummary]:
    """Return cataloged library-package OperationalModels as typed summaries.

    Mirrors the query shape used by
    ``services/operational_model_library_seed.py`` (jvspatial context-field
