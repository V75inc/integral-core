"""Operational Model create_wizard step-kind registry.

Same registry shape as ``operational_model_field_types.py`` /
``app.views.operational_model_view_types`` (Pillar 1 + Pillar 4 of the
agent-authorable substrate) — the registry lives here in core so
``operational_model_compile.py`` can validate against it, but the registry
...
No built-ins register here (unlike ``operational_model_view_types.py``,
