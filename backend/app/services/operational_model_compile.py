...
Split from ``operational_model_runtime`` per ``.planning/refactors/operational_model_runtime_split_plan.md``.
"""
...
def _normalize_field_spec(field: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if ftype == "relation":
        ...
        # Governance Policies are materialized at operational-model publish time
...
def _normalize_create_wizard(raw: Any, *, where: str) -> Optional[Dict[str, Any]]:
    ...
    for idx, s in enumerate(steps_raw):
        ...
        # same way view types are (see operational_model_wizard_steps.py's
...
def _validate_related_view_scope_placement(tier: Dict[str, Any], *, where: str) -> None:
    ...
    (the default — see ``operational_model_view_types.ViewTypeSpec.scope``).
    ...
    ``backend/app/packages/``). Track-tab candidacy is filtered
...
def _normalize_track_template_spec(
    ...
    App-attached OperationalModel. Targeted by ``relation.target_track_template``
    ...
    App-attached operational model auto-provisions on App create). A track template
    ...
    same downstream machinery (``ensure_track_attached_operational_model`` +
...
def _normalize_view_spec(spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out = {
        ...
        # (and OperationalModel authoring docs) for view-scoped slice projection.
...
def _ensure_feed_view_fallback(
    ...
    through a second compile pass when its own track-attached OperationalModel
...
def repair_stored_manifest_for_compile(manifest: Dict[str, Any]) -> Dict[str, Any]:
    ...
    this helper only runs when reading an attached OperationalModel from the DB.
    """
    import copy
...
def _canonical_manifest_base(raw: Dict[str, Any]) -> Dict[str, Any]:
    """Require operational model schema v2; empty input becomes a minimal track-scope shell.

    Phase 10 Plan 10-03 (MANIFEST-V2-01): v1 manifests are explicitly REJECTED
    via ``OperationalModelV1RejectedError`` carrying the canonical upgrade-path
    ...
    submitted = raw.get("operational_model_schema_version")
    if submitted != SCHEMA_VERSION:
        ...
        if submitted == 1:
            raise OperationalModelV1RejectedError(submitted_version=submitted)
        raise OperationalModelValidationError(
            message=(
                f"manifest must set operational_model_schema_version to "
...
#
# Each parser raises ``OperationalModelValidationError`` on shape failures,
...
def _parse_manifest_skills(
    ...
    *,
    ...
    ``OperationalModelValidationError`` at compile time. The merge-time
    ...
    *,
    ...
    Per ``app_bundles_v1.md §4.2``. Each agent normalizes to:
      ...
      - ``skills`` (list of skill keys — all must exist in
        ``declared_skill_keys``; raises OperationalModelValidationError if any
...
def _parse_manifest_queries(
    ...
    *,
    ...
    """Normalize ``app.queries[]`` — QuerySpec fixed-template OR ADR-012 handler/tool.

    Each entry must match exactly one shape:
    - QuerySpec broker: ``handler_key`` and/or ``query_template`` (no ``handler_ref``/``tool``)
    - ADR-012: ``handler_ref`` and/or ``tool`` (no ``handler_key``/``query_template``)
    """
    ...
    for idx, entry in enumerate(raw):
        ...
        has_query_template = ed.get("query_template") is not None
        is_queryspec = bool(handler_key) or has_query_template
        is_adr012 = bool(handler_ref) or bool(tool)

        if is_queryspec and is_adr012:
            raise OperationalModelValidationError(
                message=(
                    f"{where}[{key!r}] mixes QuerySpec fields "
                    "(handler_key/query_template) with ADR-012 fields "
                    "(handler_ref/tool) — pick one shape"
                )
            )
        if not is_queryspec and not is_adr012:
            raise OperationalModelValidationError(
                message=(
                    f"{where}[{key!r}] requires either handler_key/query_template "
                    "(QuerySpec) or handler_ref/tool (ADR-012)"
                )
            )
        ...
        if is_adr012:
            if handler_ref and tool:
                raise OperationalModelValidationError(
                    message=(
                        f"{where}[{key!r}] declares both handler_ref and tool — "
                        "pick one"
                    )
                )
            out.append(
        ...
        unknown = set(ed) - queryspec_allowed_keys
        if unknown:
            raise OperationalModelValidationError(
...
def _parse_manifest_tools(
    ...
    *,
    ...
    raises OperationalModelValidationError.
    """
...
def _parse_manifest_unstaged_tracks(
    ...
    *,
    ...
    for idx, raw_key in enumerate(keys):
        ...
        if not key:
            raise OperationalModelValidationError(message=f"{where}[{idx}] is empty")
...
def compile_canonical_manifest(
    *,
    ...
    Input must use ``operational_model_schema_version: 2`` and ``scope`` of
    ``track`` or ``app``. An empty object is treated as a minimal empty
    track-scope manifest. v1 inputs raise ``OperationalModelV1RejectedError``
    ...
    try:
        ...
        if scope == "track":
            ...
            # sub-manifest (inline ``profile`` or library ``operational_model_ref``)
            ...
            for app_entry in apps_in:
                ...
                operational_model_ref = ae.get("operational_model_ref")
                if not inline and not operational_model_ref:
                    raise OperationalModelValidationError(
                        message=(
                            f"workspace.apps[{slug}]: one of operational_model_ref or "
                            "profile required"
                        )
                    )
                if inline and operational_model_ref:
                    raise OperationalModelValidationError(
                        message=(
                            f"workspace.apps[{slug}]: provide only one of "
                            "operational_model_ref or profile"
                        )
                    )
                if (
                    inline
...
def find_app_track_template_spec_by_key(
    ...
    App-attached OperationalModel's compiled manifest.
    """
    if canonical.get("scope") != "app":
        return None
...
def normalize_entry_type_form_schema(
    ...
    # node's form_schema from an updated manifest spec (operational_model_
