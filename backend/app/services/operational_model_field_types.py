"""Operational Model field-type registry.

Pillar 1 of the agent-authorable substrate. Replaces the hardcoded
``VALID_FIELD_TYPES`` set in ``operational_model_runtime`` with an extensible
...
``operational_model_runtime.validate_and_materialize_entry_custom_fields`` for
...
class FieldTypeSpec:
    """Declarative spec for a operational-model field type.
...
def make_composite_spec(
    *,
    ...
    Used by ``operational_model_runtime.compile_canonical_manifest`` to
...
# ---------------------------------------------------------------------------
#
# Validation/coercion stays inline in operational_model_runtime for the
...
#   3. The introspection endpoint (``GET /api/operational-model-substrate``) and
...
_BUILTIN_FIELD_TYPES: List[FieldTypeSpec] = [
    ...
    # ``_sync_member_ref_edges`` in ``operational_model_graph.py``.
    FieldTypeSpec(
        type="member",
        base="relation",
        label="Member",
