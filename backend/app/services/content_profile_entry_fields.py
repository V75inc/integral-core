"""Entry field validation and materialization for content profiles."""
...
async def validate_and_materialize_entry_custom_fields(
    *,
    ...
    for f in fields:
        ...
        # content_profile_graph.py.
        if ftype == "member":
            from app.services.content_profile_member_field import (
        ...
        if want not in allowed:
            raise BadRequestError(
                message=(
                    ...
                    f"{entry_type.name!r} per the content profile taxonomy"
                )
            )
