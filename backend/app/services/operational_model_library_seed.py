"""Generic upsert for catalogued library OperationalModel packages (modular seeding).

Domain-specific manifests live under ``app/packages/`` as YAML files, loaded by
``operational_model_loader.load_library_operational_models``. This module only compares
canonical manifests and creates/updates ``OperationalModel`` rows — no
...
from app.services.operational_model_loader import (  # noqa: F401 — backward-compat alias
...
async def upsert_seeded_library_packages(
    *,
    ...
    """Create or refresh each spec under the OperationalModels registry."""
    for spec in specs:
        await _upsert_one(
...
async def _upsert_one(
    *,
    ...
    if not found:
        lib_cp = await OperationalModel.create(
            name=spec.name,
        ...
        logger.info("Seeded library operational model %r", spec.name)
        return
    ...
    if needs_update:
        ...
        await lib_cp.save()
        logger.info(
            "Updated library operational model %r to version %s",
            spec.name,
            spec.version,
        )
    await ensure_catalog_edge(catalog_registry, lib_cp)
