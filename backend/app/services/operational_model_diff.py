"""Structural diff + entry-impact for OperationalModel drafts.
...
def _candidate_track_tier(
    ...
    attached operational model.
    """
    from app.services.operational_model_compile import _slug, compile_canonical_manifest
...
async def compute_entry_impact_for_attached(
    *,
    ...
    For track-attached operational models, finds the owning Track via the
    ``HAS_OPERATIONAL_MODEL`` reverse edge and runs :func:`compute_entry_impact`
    once. For app-attached operational models, repeats per child Track.
    """
