...
Pipeline:
    ...
    1. Pre-flight (no writes)
        - Bundle exists in ``load_library_profiles()``.
    ...
    2. Writes (single unit)
        - For each App in declaration order:
            * Create ``App`` node with ``source_profile_slug = bundle_slug``.
            * Wire the workspace branch via ``catalog_app`` (I-GRAPH-01 known
              wiring helper — attaches the App under the workspace's ``Apps``
              registry and ensures the attached ContentProfile).
            * Apply the sub-manifest to the App's attached ContentProfile
              via ``merge_library_manifest_into_content_profile`` and
...
D4 wires this service into ``POST /workspaces?profile_slug=<slug>``.
"""
...
async def init_workspace_from_profile(
    ...
    Args:
        ...
        profile_slug: Library bundle slug to provision from.
    ...
    Raises:
        ...
        WorkspaceInitConflict: Workspace already has an
            ``applied_profiles`` entry for ``profile_slug``.
    ...
    spec = next(
        (s for s in load_library_profiles() if s.slug == profile_slug),
        None,
    )
    if spec is None:
        raise WorkspaceInitValidationError(f"bundle '{profile_slug}' not found")

    manifest = spec.manifest or {}
    if manifest.get("scope") != "workspace":
        raise WorkspaceInitValidationError(
            f"bundle '{profile_slug}' is not scope=workspace"
        )

    if any(
        (e or {}).get("slug") == profile_slug
        for e in (workspace.applied_profiles or [])
    ):
        raise WorkspaceInitConflict(
            f"workspace already provisioned from '{profile_slug}'"
        )

    if not spec.signature_verified:
        raise WorkspaceInitValidationError(
            f"bundle '{profile_slug}' signature failed " f"({spec.signature_reason})"
        )
    ...
    if not apps_decl:
        raise WorkspaceInitValidationError(
            f"bundle '{profile_slug}' workspace.apps[] is empty"
        )
    ...
    try:
        for app_entry in apps_decl:
            ...
            profile_ref = app_entry.get("profile_ref")
            if (
                not app_source_slug
                and isinstance(profile_ref, str)
                and not profile_ref.startswith("./")
            ):
                app_source_slug = profile_ref.strip()
            app = await _create_workspace_app(
                workspace=workspace,
                name=app_name,
                actor_id=actor_id,
                source_profile_slug=app_source_slug or profile_slug,
            )
            ...
            if inline:
                ...
                if app_source_slug and app_source_slug != profile_slug:
                    ...
                    sibling = next(
                        (
                            s
                            for s in load_library_profiles()
        ...
        # ``content_profile_merge.materialize_app_relations`` (which handles
...
def _resolve_sub_manifest(
    ...
    """Resolve an app entry's sub-manifest from inline / profile_ref.

    ``profile`` (inline) takes precedence. ``profile_ref`` may be:
      - ``"./relative/path.yaml"`` — read from disk under ``spec.bundle_dir``.
      - ``"sibling-slug"`` — resolved from ``load_library_profiles()``.
    ...
    if isinstance(profile_ref, str) and profile_ref.startswith("./"):
        rel = profile_ref[2:]
        if spec.bundle_dir is None:
            raise WorkspaceInitValidationError(
                f"bundle '{spec.slug}' app '{app_entry.get('slug')}' "
                f"declares relative profile_ref but bundle has no bundle_dir"
            )
    ...
    sibling = next(
        (s for s in load_library_profiles() if s.slug == profile_ref),
        None,
    )
    if sibling is None:
        raise WorkspaceInitValidationError(
            f"app '{app_entry.get('slug')}' profile_ref " f"'{profile_ref}' not found"
        )
...
async def _create_workspace_app(
    *,
    ...
    and provisions the attached ContentProfile in the same call.
...
async def _apply_app_submanifest(
    *,
    ...
    attached ContentProfile via the same merge-library + track
    ...
    from app.services.content_profile_runtime import SCHEMA_VERSION

    try:
        attached_cp = await get_app_attached_content_profile(app)
        if attached_cp is None:
            raise RuntimeError(
                f"app {app.id} has no attached content profile to merge into"
            )
        ...
        if "content_profile_schema_version" not in normalized:
            normalized["content_profile_schema_version"] = SCHEMA_VERSION
    ...
    *,
    ...
    For each declared ``workspace.cross_app_relations[]`` entry:
      ...
      2. Walk ``App —CONTAINS→ Track`` (filtered by ``Track.template_id``)
         ...
         sub-manifest.
      3. Walk ``Track —HAS_CONTENT_PROFILE→ ContentProfile —CONTAINS→
    ...
    ``content_profile_merge.materialize_app_relations`` so future
    ...
    for rel in cross_app_relations:
        ...
        tgt_tcp = await get_track_attached_content_profile(tgt_track)
...
async def _find_entry_type_by_key(content_profile: Any, et_key: str) -> Any:
    """Find a ContentProfile-contained EntryType whose case-folded name matches ``et_key``."""
    from app.models.edges import CONTAINS
    from app.services.content_profile_runtime import slug_manifest_key
