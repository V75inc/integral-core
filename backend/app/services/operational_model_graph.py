"""Graph materialization helpers for operational models (edges, anchors, taxonomy)."""
...
from app.models.nodes import App, Entry, EntryType, OperationalModel, Tag, Track, View
...
def is_space_track_template_profile(operational_model: OperationalModel) -> bool:
    """True when CP is a shared anchor-track template (not a dedicated track CP)."""
    manifest = operational_model.manifest or {}
...
async def materialize_anchor_track(
    *,
    ...
    caller does NOT supply an explicit value. The newly-created Track:
      ...
      - receives the template's OperationalModel BY REFERENCE via
        ``HAS_OPERATIONAL_MODEL`` + ``Track.attached_operational_model_id`` scalar
        (researcher note: HAS_OPERATIONAL_MODEL is polymorphic-target, so many
        Tracks may share one CP — the template OperationalModel node is
    ...
    Raises ``BadRequestError`` when:
      ...
      - the App-attached OperationalModel is missing or does not declare
    ...
    # 2) Resolve the App-attached OperationalModel + its compiled manifest.
    app_cp = await get_app_attached_operational_model(app_node)
    if app_cp is None or not app_cp.manifest:
        raise BadRequestError(
            message=(
                f"Anchor auto-provision requires an App-attached OperationalModel "
    ...
    canonical = compile_canonical_manifest(
        manifest=_as_dict(app_cp.manifest, where="app operational model manifest")
    )
    ...
    # Policies persisted at operational-model publish time live under
    ...
    # subjects). See policy_registry.materialize_governance_policies_for_operational_model.
    ...
    # 3) Resolve-or-materialize the template OperationalModel node. The template
    ...
    template_cp = await _resolve_or_create_template_operational_model(
        app_node=app_node,
        template_key=template_key,
    ...
    anchor_track = await Track.create(
        ...
        attached_operational_model_id=template_cp.id,
        created_at=now,
        updated_at=now,
    )
    ...
    # 7) Wire HAS_OPERATIONAL_MODEL (Track → CP) by reference and TEMPLATED_FROM
    # (Track → CP) for lineage. attached_operational_model_id was set on
    # create — keep the scalar + edge in lockstep.
    await anchor_track.connect(template_cp, edge=HAS_OPERATIONAL_MODEL, attached_at=now)
    ...
    try:
        ...
        await emit_change_event(
            ...
            after={
                ...
                "attached_operational_model_id": anchor_track.attached_operational_model_id,
...
async def _resolve_or_create_template_operational_model(
    *,
    app_node: App,
    template_key: str,
    ...
    """Return the per-App template OperationalModel for ``template_key``.
    ...
    ``OperationalModel.name`` for lineage and keeps the discriminator inside
    the manifest payload (no node-schema change required).

    By-reference contract: subsequent Tracks anchored from the SAME
    ``template_key`` in this App reuse the SAME OperationalModel node —
    ...
    template_manifest: Dict[str, Any] = {
        "operational_model_schema_version": SCHEMA_VERSION,
        "scope": "track",
        "track": {
    ...
    # Walk the App's outgoing CONTAINS edges to OperationalModel children.
    # (Cataloging via CONTAINS is the existing pattern — see
    # ensure_app_attached_operational_model for the attached-profile case.)
    candidates = await app_node.nodes(
        edge=["CONTAINS"], direction="out", node=["OperationalModel"]
    )
    for cand in candidates:
        if not isinstance(cand, OperationalModel):
            continue
        ...
        if discriminator_match:
            ...
            # _materialize_template_operational_model_nodes raised (e.g. a
            ...
            if manifest_stale or needs_node_heal:
                ...
                cand.updated_at = now
                await cand.save()
                await _materialize_template_operational_model_nodes(cand)
            return cand
    ...
    # a OperationalModel with zero cataloged Views, so its inline related_views
    ...
    await _materialize_template_operational_model_nodes(template_cp)
    return template_cp


async def _materialize_template_operational_model_nodes(
    template_cp: OperationalModel,
) -> None:
    """Create real EntryType/Tag/View nodes on a template CP from its own manifest.

    Mirrors ``apply_space_track_spec_to_track``'s shim-merge pattern (operational_model_merge.py)
    ...
    # ``materialization`` discriminator (_resolve_or_create_template_operational_model's
    ...
    *,
    ...
    for g in groups:
        ...
        for t in _as_list(gd.get("tags"), where=f"taxonomy.{gkey}.tags"):
            ...
            await operational_model.connect(tag, edge=CONTAINS)
...
async def sync_attached_manifest(operational_model: OperationalModel) -> None:
    """Rebuild the manifest on an attached OperationalModel from its materialized nodes.
    ...
    For track-attached operational models, the manifest ``track`` tier is rebuilt from:
      - EntryType nodes connected via CONTAINS
      - Tag nodes connected via CONTAINS
      - View nodes cataloged under the Views registry

    For App-attached operational models, the manifest ``app_node`` tier is rebuilt from:
      - Track-template OperationalModel nodes connected via DEFINES_TRACK_PROFILE
    ...
    """
    existing = operational_model.manifest or {}
    scope = str(existing.get("scope") or operational_model.scope or "track")
    ...
    if scope == "track":
        ...
        attached_tracks = await operational_model.nodes(
        ...
        for attached_track in attached_tracks:
            ...
            for et in orphans or []:
                ...
                if not et_id or et_id in seen_et_ids:
                    continue
                ctx = await operational_model.get_context()
                if not await ctx.find_edges_between(
                    operational_model.id, et_id, edge_class=CONTAINS
                ):
                    await operational_model.connect(
                        et, edge=CONTAINS, added_at=sync_now
                    )
                entry_types.append(et)
                seen_et_ids.add(et_id)
        tags = await operational_model.nodes(edge=[CONTAINS], node=["Tag"])

        # Resolve Views registry → cataloged Views
        vregs = await operational_model.nodes(edge=[Edge], node=["Views"])
        views: List[View] = []
        if vregs:
        ...
        template_cps = await operational_model.nodes(
    ...
    operational_model.manifest = manifest
    operational_model.updated_at = datetime.now(timezone.utc).isoformat()
