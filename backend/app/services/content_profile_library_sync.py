"""Shared disk-to-catalog sync for library profile bundles.
...
def load_library_profiles_with_issues_cached() -> (
    tuple[Sequence[LibraryProfileSpec], Sequence[ProfileLoadIssue]]
):
    """Return library specs parsed once per process (test session).

    Invalidates when ``content_profile_loader._PROFILES_ROOT`` or
    ``INTEGRAL_PACKAGE_PATHS`` / ``INTEGRAL_CORE_ONLY`` change.
    """
    from app.services.content_profile_loader import _PROFILES_ROOT
    ...
    if _CACHED_LIBRARY_SPECS is None or _CACHED_PROFILES_ROOT != cache_key:
        _CACHED_LIBRARY_SPECS = load_library_profiles_with_issues()
        _CACHED_PROFILES_ROOT = cache_key
    return _CACHED_LIBRARY_SPECS


def reset_library_profiles_cache_for_testing() -> None:
...
async def _is_library_profile_referenced(content_profile_id: str) -> bool:
    tracks = await Track.find({"context.library_merge_source_id": content_profile_id})
    if tracks:
        return True
    apps = await App.find({"context.library_merge_source_id": content_profile_id})
    return bool(apps)
...
async def reconcile_removed_seeded_packages(
    *,
    ...
    for slug in removed_slugs:
        rows = await ContentProfile.find(
            {
        ...
        for cp in matches:
            if getattr(cp, "workspace_id", None):
                ...
                continue
            referenced = await _is_library_profile_referenced(cp.id)
...
async def sync_library_catalog_from_disk(
    *,
    ...
    if use_cached_specs:
        specs, issues = load_library_profiles_with_issues_cached()
    else:
