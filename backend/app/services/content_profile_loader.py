"""YAML-based library profile loader (v3 — Phase B2).

Walks ``backend/app/profiles/*/profile.yaml`` and returns
...
v3 additions (Spec §4.2):
  ...
  - Signature gate via content_profile_signature.verify_bundle_signature
...
empty/None. The emitted ``manifest.content_profile_schema_version`` is pinned
at ``2`` so the v2 canonical-manifest validator keeps accepting bundles
regardless of whether the YAML declares ``integral_profile_version: 2`` or
...
# Absolute path to the first configured package root (default: app/profiles/).
# Tests may monkeypatch this module attribute; prefer package_paths.resolve_*.
_PROFILES_ROOT = Path(__file__).parent.parent / "profiles"


@dataclass(frozen=True)
class LibraryProfileSpec:
    """One library package loaded from a profile.yaml file (v3)."""
...
def _assemble_manifest(raw: Dict[str, Any]) -> Dict[str, Any]:
    """Reconstruct the manifest dict from a parsed profile.yaml.
    ...
    The emitted ``content_profile_schema_version`` stays pinned at ``2`` so
    the existing canonical-manifest validator (``_canonical_manifest_base``)
    accepts it regardless of whether the YAML declares
    ``integral_profile_version: 2`` (legacy) or ``3`` (Phase B bundles). The
...
def load_library_profiles(
    ...
    *,
    ...
    """Load library profile specs, discarding per-package load issues.

    Convenience wrapper around ``load_library_profiles_with_issues`` for callers
...
def load_library_profiles_with_issues(
    profiles_root: Path | None = None,
    *,
    verify_signatures: bool = True,
    package_paths: Sequence[Path] | None = None,
    ...
    """Walk package roots for ``*/profile.yaml`` and return one spec per package.

    Packages whose profile.yaml cannot be parsed are logged and skipped —
    a bad YAML file must not crash startup.

    v3 gates (Spec §4.2):
      - I-BUNDLE-01: bundles without profile.yaml are silently skipped (handled
        by the ``*/profile.yaml`` glob).
      ...
      - Signature gate: when ``verify_signatures`` is on and a pubkey is
        configured via ``INTEGRAL_PROFILE_PUBKEY``, a bundle that ``ships_python``
    ...
    if profiles_root is not None:
        ...
        default_root = Path(__file__).parent.parent / "profiles"
    ...
    if not existing_roots:
        logger.warning(
            "profiles roots %s do not exist — no library packages loaded", roots
        )
        _issue(
            issues,
            slug="",
            code="profiles_root_missing",
            message=f"profiles roots {roots} do not exist",
            level="warning",
        )
        return specs, issues

    pubkey = os.environ.get("INTEGRAL_PROFILE_PUBKEY") if verify_signatures else None

    profile_paths: List[Path] = []
    for root in existing_roots:
        profile_paths.extend(sorted(root.glob("*/profile.yaml")))
        ...
        try:
            ...
            if slug in seen_slugs:
                ...
                _issue(
                    ...
                    slug=slug,
                    code="duplicate_slug",
                    message=f"duplicate package slug already loaded; skipping {profile_path}",
                    level="warning",
                    profile_path=str(profile_path),
                )
                continue
            ...
            if verify_signatures:
                ...
                from app.services.content_profile_signature import SignatureResult
    ...
    logger.info(
        "Loaded %d library profiles from %s (core_only=%s)",
        len(specs),
        existing_roots,
        core_only,
    )
