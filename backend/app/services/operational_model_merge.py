"""Merge library manifests or track templates into attached OperationalModels."""
...
class _LibraryManifestSource(Protocol):
    """Anything that exposes ``manifest`` for merge (OperationalModel or in-memory shim)."""
...
def _merge_track_tier_into_manifest(
    ...
    from app.services.operational_model_runtime import _slug
    ...
    # (_resolve_or_create_template_operational_model) keeps matching.
    prior_mat = (
...
async def _resolve_library_dependency_chain(
...
) -> List[OperationalModel]:
    resolved: List[OperationalModel] = []
    visiting: Set[str] = set()
    visited: Set[str] = set()

    async def _lookup(dep_id: str) -> Optional[OperationalModel]:
        dep = await OperationalModel.get(dep_id)
        if dep and getattr(dep, "library_package", False):
            return dep
        packs = await OperationalModel.find({"context.library_package": True})
    ...
    async def _visit(cp: OperationalModel) -> None:
        if cp.id in visited:
            return
        ...
        for dep in deps:
            ...
            if not dep_cp:
                raise BadRequestError(
                    message=f"Missing dependency '{dep_id}' for operational model '{cp.id}'"
                )
            await _visit(dep_cp)
...
async def verify_track_template_in_app(
    app_id: str, template_id: str
) -> OperationalModel:
    ...
    sacp = await get_app_attached_operational_model(sp)
    if not sacp:
        raise BadRequestError(message="App has no attached operational model")
    ...
    t = await OperationalModel.get(template_id)
    if not t:
        raise ResourceNotFoundError(message="Template not found")
    return t


async def merge_template_operational_model_into_track(
    ...
    if not tcp:
        raise BadRequestError(message="Track has no attached operational model")
    now = datetime.now(timezone.utc).isoformat()
    ...
    tvreg = await get_or_create_views_registry_for_operational_model(
...
async def materialize_app_relations(
    ...
    # Resolve the owning App from the attached operational model
    ...
    for rel in relations:
        ...
        # Get the track-attached operational models and their EntryTypes
        source_tcp = await get_track_attached_operational_model(source_track)
        target_tcp = await get_track_attached_operational_model(target_track)
...
async def merge_library_manifest_into_operational_model(
    library_cp: _LibraryManifestSource,
    target_cp: OperationalModel,
    ...
    *,
    ...
    # library OperationalModel was compiled with ``is_public_catalog=False`` (e.g.
    # imported by a non-marketplace pathway), a manifest carrying
    # ``package.publisher_tier == "public_catalog"`` cannot reach an attached
    # OperationalModel if any declared skill is ``kind: custom``.
    ...
    if for_space and canonical.get("scope") == "app" and track is None:
        ...
        # (merge_library_manifest_into_operational_model mirror check).
        ...
        # dropped from the rebuilt app block, so an attached operational model lost its
    ...
    for spec in view_specs:
        ...
        if track:
            new_v = await View.create(
                ...
                track_id=track.id,
                operational_model_id=target_cp.id,
...
async def preview_effective_app_manifest_after_library_merge(
    *,
    ...
    This intentionally delegates to ``merge_library_manifest_into_operational_model``
...
async def refresh_app_track_template_materialization(
    ...
    """Re-merge one ``app.track_templates[]`` spec onto its template OperationalModel.
