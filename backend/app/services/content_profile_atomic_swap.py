"""Atomic draft → published promotion for ContentProfiles.
...
Pillar 2 of the agent-authorable substrate. Promotion is two-stage:
  ...
  2. Swap: copy the draft's manifest onto the published CP node, bump
     ...
     with the manifest, and emit a single ``content_profile.publish`` change
     event.
...
Track or App references it via ``attached_content_profile_id``); only the
...
EntryTypes / Tags / Views the profile defines.
...
async def fork_draft(
    *,
    published: ContentProfile,
    actor_id: Optional[str] = None,
) -> ContentProfile:
    """Fork a published CP into a new draft sibling.

    The draft is its own ContentProfile node — not the published one. It is
    not attached to any Track or App via ``HAS_CONTENT_PROFILE``; the agent
    ...
    draft = await ContentProfile.create(
    ...
    # ContentProfile subgraph. publish_draft preserves the draft node
...
async def publish_draft(
    *,
    draft: ContentProfile,
    published: Optional[ContentProfile] = None,
    actor_id: Optional[str] = None,
    ...
    if published is None:
        ...
        published = await ContentProfile.get(draft.draft_of_id)
        if published is None:
    ...
    if run_migrations and not force:
        try:
            ...
            raise RuntimeError(
                ...
                "(content_profile_diff / migrations.reject_gate failed to "
    ...
    await emit_change_event(
        ...
        action="content_profile.publish",
        ...
        scope=f"content_profile:{published.id}",
    )
    ...
    try:
        ...
        await materialize_governance_policies_for_content_profile(
...
async def _snapshot_publish_state(
    published: ContentProfile, draft: ContentProfile
) -> Dict[str, Any]:
