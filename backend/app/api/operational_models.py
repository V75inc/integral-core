"""Library and attached OperationalModel APIs.
...
def _is_visible_library_profile(profile: OperationalModel) -> bool:
...
async def list_library_operational_models(
    ...
    """List cataloged library content packages (Phase 6 MCP-04 — ``integral_list_models``).
...
@endpoint(
    "/operational-models/{operational_model_id}",
    methods=["GET"],
    auth=True,
    tags=["Operational Models"],
)
async def get_library_operational_model(
    request: Request, operational_model_id: str
) -> Dict[str, Any]:
...
@endpoint(
    "/workspaces/{workspace_id}/operational-models",
    methods=["GET"],
    auth=True,
    tags=["Operational Models"],
)
async def list_workspace_operational_models(
    request: Request,
    workspace_id: str,
) -> Dict[str, Any]:
    ...
    return {
        "operational_models": [await export_node(cp) for cp in cps],
        "total": len(cps),
...
@endpoint(
    "/operational-models", methods=["POST"], auth=True, tags=["Operational Models"]
)
async def publish_operational_model(
    request: Request,
    name: str,
    description: str = "",
    ...
    """Publish a new workspace-private library operational model package."""
    user_id = resolve_principal_id(request)
    if not user_id:
        raise MissingAuthenticationError(message="Authentication required")
    if not workspace_id:
    ...
    reg = await OperationalModels.get(OPERATIONAL_MODELS_REGISTRY_ID)
    if not reg:
        raise ResourceNotFoundError(message="Operational Models registry missing")
    now = utc_now_iso()
    ...
    return {
        ...
        "message": "Operational Model published",
    }
...
async def update_library_operational_model(
    request: Request,
    ...
    if is_draft:
        ...
        if not await can_publish_operational_models_under_workspace(
...
async def deprecate_library_operational_model(
    request: Request,
    operational_model_id: str,
) -> Dict[str, Any]:
    ...
    await emit_change_event(
        ...
        resource_type="OperationalModel",
        resource_id=operational_model_id,
        before=prior_snapshot,
        after=None,
        scope=f"user:{user_id}",
    )
    ...
    return {
        "message": "Operational Model deprecated",
        "operational_model_id": operational_model_id,
    }


@endpoint(
    "/operational-models/validate",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def validate_operational_model_manifest(
    request: Request,
    manifest: Optional[Dict[str, Any]] = None,
    manifest_yaml: Optional[str] = None,
    scope: Optional[str] = None,
...
@endpoint(
    "/tracks/{track_id}/operational-model/entry-types",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def add_entry_type_to_track_profile(
    ...
    """Add an EntryType to the track-attached OperationalModel (updates manifest + creates node)."""
    user_id = resolve_principal_id(request)
    if not user_id:
        raise MissingAuthenticationError(message="Authentication required")
    ...
    if not cp:
        raise ResourceNotFoundError(message="Track has no attached operational model")
...
@endpoint(
    "/tracks/{track_id}/operational-model/entry-types/{entry_type_id}",
    methods=["DELETE"],
    auth=True,
    tags=["Operational Models"],
)
async def remove_entry_type_from_track_profile(
    ...
    """Remove an EntryType from the track-attached OperationalModel (updates manifest + deletes node)."""
    user_id = resolve_principal_id(request)
    if not user_id:
        raise MissingAuthenticationError(message="Authentication required")
    _decision = await policy_evaluate(
...
@endpoint(
    "/tracks/{track_id}/operational-model/views",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def add_view_to_track_profile(
    ...
    """Add a View to the track-attached OperationalModel (updates manifest + creates node)."""
    user_id = resolve_principal_id(request)
    if not user_id:
        raise MissingAuthenticationError(message="Authentication required")
    _decision = await policy_evaluate(
    ...
    from app.views import operational_model_view_types as _vtr
...
@endpoint(
    "/tracks/{track_id}/operational-model/views/{view_id}",
    methods=["DELETE"],
    auth=True,
    tags=["Operational Models"],
)
async def remove_view_from_track_profile(
    ...
    """Remove a View from the track-attached OperationalModel (updates manifest + deletes node)."""
    user_id = resolve_principal_id(request)
    if not user_id:
        raise MissingAuthenticationError(message="Authentication required")
    _decision = await policy_evaluate(
...
@endpoint(
    "/tracks/{track_id}/operational-model/detach-library",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def detach_library_from_track_profile(
    ...
    """Remove library provenance from a track-attached OperationalModel without removing materialized elements.
...
@endpoint(
    "/tracks/{track_id}/operational-model/revert-customizations",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def revert_track_profile_customizations(
    ...
    """Revert local customizations on a track-attached OperationalModel back to the last merged library state.
    ...
    lib_cp = await OperationalModel.get(lib_id)
    if not lib_cp or not getattr(lib_cp, "library_package", False):
        raise ResourceNotFoundError(
            message="Referenced library Operational Model not found"
        )

    # Remove all current materialized nodes under the attached operational model
    cp = await get_track_attached_operational_model(track)
    if not cp:
        raise ResourceNotFoundError(message="Track has no attached operational model")
    ...
    # D-05 single emission path. Sync inline emit before HTTP response (D-06).
    # Plan 07-02 — switch to operational_model.update action + details.revert so
...
@endpoint(
    "/operational-models/from-track/{track_id}",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def derive_library_profile_from_track(
    ...
    """Derive a new library OperationalModel from a Track's attached operational model.

    The attached operational model's manifest is used as the basis for the new library
    ...
    return {
        "operational_model": await export_node(derived),
...
@endpoint(
    "/operational-models/from-app/{app_id}",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def derive_library_profile_from_space(
    ...
    """Derive a new library OperationalModel from an App's attached operational model.
    ...
    if not cp:
        raise ResourceNotFoundError(message="App has no attached operational model")
...
# OperationalModels. Per Phase 7 CONTEXT post-research lock #12, BOTH
# endpoints are NON-CASCADING: they only mutate the App's directly-
# attached OperationalModel and the App.library_merge_source_id field.
# Track-template OperationalModel rows reachable via the DEFINES_TRACK_PROFILE
...
@endpoint(
    "/apps/{app_id}/operational-model/detach-library",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def detach_library_from_space_profile(
    ...
    """Remove library provenance from an App-attached OperationalModel.

    Drops ``library_merge_source_id`` on the App and clears
    ``manifest.package.source_library_{id,name}`` on the attached
    OperationalModel while preserving every materialized EntryType / Tag /
    View / track-template OperationalModel (I-LIB-04, non-cascading).
...
@endpoint(
    "/apps/{app_id}/operational-model/revert-customizations",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def revert_space_profile_customizations(
    ...
    """Revert local customizations on an App-attached OperationalModel.
    ...
    track-template OperationalModel rows reachable from the App CP via
    ``DEFINES_TRACK_PROFILE`` are PRESERVED. Child Tracks created from
    those templates keep their independent track-attached operational models.
...
async def _resolve_cp_edit_permission(
    *,
    ...
    # the underlying track/app. Mirrors operational_model_authoring._user_can_edit_cp.
    ...
    if cp.scope == "track":
        for cid in candidate_cp_ids:
            owner_tracks = await Track.find(
                {"context.attached_operational_model_id": cid}
            )
...
@endpoint(
    "/operational-models/{operational_model_id}/migration-status",
    methods=["GET"],
    auth=True,
    tags=["Operational Models"],
)
async def get_operational_model_migration_status(
    request: Request,
    operational_model_id: str,
) -> Dict[str, Any]:
...
@endpoint(
    "/operational-models/{operational_model_id}/retry-migration",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def retry_operational_model_migration(
    request: Request,
    operational_model_id: str,
) -> Dict[str, Any]:
...
@endpoint(
    "/operational-models/{operational_model_id}/draft",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def fork_operational_model_draft(
    request: Request,
    operational_model_id: str,
) -> Dict[str, Any]:
    """Fork a draft from a published OperationalModel."""
    user_id = resolve_principal_id(request)
    if not user_id:
        raise MissingAuthenticationError(message="Authentication required")
    cp = await OperationalModel.get(operational_model_id)
...
@endpoint(
    "/operational-models/{operational_model_id}/publish",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def publish_operational_model_draft(
    request: Request,
    operational_model_id: str,
    ...
    if not user_id:
        raise MissingAuthenticationError(message="Authentication required")
    draft = await OperationalModel.get(operational_model_id)
    if draft is None:
        raise ResourceNotFoundError(message="Operational Model not found")
    ...
    decision = await policy_evaluate(
        ...
        resource=Resource(
            ...
            id=operational_model_id,
            scope=f"operational_model:{operational_model_id}",
        ),
    )
...
@endpoint(
    "/operational-models/{operational_model_id}/diff",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def diff_operational_model(
    request: Request,
    operational_model_id: str,
    ...
    reference = await OperationalModel.get(against_id)
    if reference is None:
        raise ResourceNotFoundError(message="Reference operational model not found")
    ...
    if include_entry_impact:
        ...
        # on ``app.services.operational_model_diff.compute_entry_impact_for_attached``
        # is honoured by tests that inject synthetic impact lists.
        from app.services import operational_model_diff as _cpd
...
#
# ``POST /api/operational-models/{id}/preview-update`` is a thin wrapper that
# delegates to ``diff_operational_model`` (locked decision #3, CONTEXT
...
@endpoint(
    "/operational-models/{operational_model_id}/preview-update",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def preview_update_operational_model(
    request: Request,
    operational_model_id: str,
    against: Optional[str] = None,
    include_entry_impact: bool = True,
    ...
    """MIG-04 — alias to ``diff_operational_model``.
    ...
    """
    return await diff_operational_model(
        request=request,
        operational_model_id=operational_model_id,
        against=against,
...
@endpoint(
    "/operational-models/{operational_model_id}/discard-draft",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def discard_operational_model_draft(
    request: Request,
    operational_model_id: str,
) -> Dict[str, Any]:
...
async def get_operational_model_substrate(request: Request) -> Dict[str, Any]:
...
#   - POST /api/operational-models/author          → integral_author_model
#   - POST /api/operational-models/{id}/modify     → integral_modify_model
#   - GET  /api/operational-models?type_hint=<str> → integral_list_models
#
...
async def resolve_type_hint(hint: str) -> List[Dict[str, Any]]:
    ...
    resolution"):
      ...
      * For each ``library_package=True`` OperationalModel, build a haystack
    ...
    (``list_library_operational_models`` + ``create_track`` + ``create_app``)
    invoke it directly without an MCP round-trip.
    """
    hint_tokens = {tok for tok in hint.lower().replace("-", " ").split() if tok}
    if not hint_tokens:
        return []
    raw = await OperationalModel.find({"context.library_package": True})
    if raw is None:
        rows: list = []
    ...
    for cp in rows:
        # Drafts are not "listable" via integral_list_models — they
...
async def author_operational_model(
    ...
    """integral_author_model (MCP-04 — Phase 6 Plan 06-04 AC#3).
    ...
    Body:
      ...
      * ``as_draft`` (bool, default=True) — when True, the CP lands as a
                                        ...
                                        ``integral_list_models``); when
                                        False, lands published.

    Two-tier gate (Pitfall 5):

      1. ``policy_engine.evaluate(action="operational_model.author", scope=workspace)``
      2. ``can_publish_operational_models_under_workspace(user_id, workspace_id)``
    ...
    if not decision.allowed:
        raise InsufficientPermissionsError(message="operational_model.author denied")
    ...
    from app.services.operational_model_author import (
        author_operational_model_from_template,
    )

    manifest = await author_operational_model_from_template(
    ...
    # Catalog under OperationalModels registry so list_library_operational_models
...
@endpoint(
    "/operational-models/{operational_model_id}/modify",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def modify_operational_model(
    ...
    """integral_modify_model (MCP-04 — Phase 6 Plan 06-04 AC#4).
...
async def _publish_single_manifest(
    ...
    """Persist one manifest as a library OperationalModel node.

    Returns the created OperationalModel node.
    """
...
@endpoint(
    "/operational-models/import",
    methods=["POST"],
    auth=True,
    tags=["Operational Models"],
)
async def import_operational_model(
    request: Request,
    workspace_id: Optional[str] = None,
    preview: bool = False,
    manifest: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """Import OperationalModel manifests from a YAML/JSON file, or a ZIP archive.

    Accepts:
    - ``multipart/form-data`` with a ``file`` field (.yaml, .json, or .zip).
    - ``application/json`` body with a ``manifest`` key (single manifest only).

    ZIP archives must contain one or more ``<package-name>/operational-model.yaml``
    bundles (one level deep). Maximum ZIP size is 50 MB.

    When ``?preview=true`` the manifest(s) are validated and stats returned
    without persisting anything. In publish mode new library ``OperationalModel``
    ...
    Publish response shape:
      - Single file: ``{"operational_model": {...}, "message": "..."}``
      - ZIP archive:  ``{"published": N, "operational_models": [...], "message": "..."}``
    """
    import pathlib
    import tempfile
    import zipfile
    ...
    if filename.endswith(".zip") and raw_bytes is not None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Collect operational-model.yaml files exactly one level deep
            profile_files = sorted(tmp_path.glob("*/operational-model.yaml"))
            if not profile_files:
                raise BadRequestError(
                    message=(
                        "No operational-model.yaml files found in archive. "
                        "Expected structure: <package-name>/operational-model.yaml"
                    )
                )
            ...
            if not reg:
                raise ResourceNotFoundError(
                    message="Operational Models registry missing"
                )
            now = utc_now_iso()
            published_nodes = []
            for pf in profile_files:
            ...
            return {
                ...
                "operational_models": published_nodes,
                "message": f"{len(published_nodes)} operational model(s) imported",
            }
    ...
    return {
        "operational_model": await export_node(cp),
        "message": "Operational Model imported",
    }
