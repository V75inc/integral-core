"""Admin endpoints for operational-model bundle hot-load + introspection.

Phase C1 (spec §4.6, §9.3, §8.4 I-HOTLOAD-01).

Three admin-only endpoints under ``/api/admin/packages``:

- ``POST /admin/packages/rescan`` — re-walk ``backend/app/packages/``,
...
- ``GET /admin/packages`` — list every loaded bundle with metadata
  (signature/fingerprint/scope/skill-keys).
- ``GET /admin/packages/{slug}/installed`` — impact preview: which
...
@endpoint("/admin/packages/rescan", methods=["POST"], auth=True, tags=["Admin"])
async def rescan_packages(request: Request) -> Dict[str, Any]:
    """Re-walk the packages tree and upsert changed bundles.
    ...
    Upserts OperationalModel rows for added + updated via the existing
    ...
    async with _RESCAN_LOCK:
        ...
        cps_registry = await OperationalModels.get(OPERATIONAL_MODELS_REGISTRY_ID)
        if cps_registry is None:
            logger.warning(
                "rescan: OperationalModels registry %s not found — skipping upsert",
            ...
            [
                {
                    ...
                    "model_path": getattr(issue, "model_path", ""),
                }
        ...
        # (upserts OperationalModel rows) so it MUST surface a ChangeEvent even
        ...
        await emit_change_event(
            actor_kind="human",
            actor_id=actor_id,
            action="operational_model.rescan",
            resource_type="OperationalModel",
            resource_id="library:rescan",
            before=None,
            after=None,
            scope="library:operational_models",
            details={
                "added": added,
                "updated": updated,
                "removed": removed,
...
@endpoint("/admin/packages", methods=["GET"], auth=True, tags=["Admin"])
async def list_loaded_profiles(request: Request) -> Dict[str, Any]:
    """List every bundle currently loadable from ``app/packages/``.
...
@endpoint(
    "/admin/packages/{slug}/installed", methods=["GET"], auth=True, tags=["Admin"]
)
