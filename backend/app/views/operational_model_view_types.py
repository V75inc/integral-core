"""Operational Model view-type registry.

Pillar 1 + Pillar 4 of the agent-authorable substrate. Replaces the hardcoded
``VALID_VIEW_TYPES`` set in ``operational_model_runtime`` with an extensible
...
class ViewTypeSpec:
    """Declarative spec for a operational-model view type / widget capability.
    ...
    # placement. See `operational_model_compile.py`'s related_views placement
...
# UI Packs Standard (docs/operational-models/UI_PACKS.md) requires every NEW
...
def register_view_type(spec: ViewTypeSpec, *, override: bool = False) -> None:
    ...
    if (
        ...
        raise ValueError(
            ...
            "'pack-name/view-name' (see docs/operational-models/UI_PACKS.md)"
        )
