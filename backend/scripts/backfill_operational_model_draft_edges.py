...
Idempotent. For every persisted OperationalModel with ``draft_of_id``
...
async def backfill_operational_model_draft_edges(*, dry_run: bool = False) -> dict:
    ...
    for cp in cps:
        ...
        if not parent_id:
            continue
        parent = await OperationalModel.get(parent_id)
        if parent is None:
            stats["skipped"] += 1
            continue
...
async def _main(dry_run: bool) -> None:
    ...
    init_prime_db()
    stats = await backfill_operational_model_draft_edges(dry_run=dry_run)
    print(
