...
- **`observation_max_chars` 4000 → 12000, `stale_observation_max_chars` → 4000, `observation_full_recent` 3 → 5** on the orchestrator (`agent/agents/integral/integral_agent/agent.yaml`). Stale must stay ≤ recent (an inverted 18000 caused quadratic prompt growth on long scaffolds). **Note these apply only where `JVAGENT_UPDATE_MODE=source`** — under `merge` (prod) the persisted action node keeps its existing values, and the server now warns about that at boot.
