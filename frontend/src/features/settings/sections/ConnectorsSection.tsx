 ...
 *
 * `conflict_policy` is rendered as a read-only StatusPill (per A5 —
 * derived from the SyncConnector subclass registry, NEVER editable).
 */
import { useEffect, useState } from 'react';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { ArrowLeft, Cable, HeartPulse, List, Pencil, PlugZap, Plus, RefreshCw, Settings, Trash2 } from 'lucide-react';
...
import { McpToolsInspector, toolCountLabel } from '../connectors/McpToolsInspector';
...
export function ConnectorsSection() {
  ...
  const instanceMeta = (c: ConnectorResponse) => {
    ...
    if (c.subclass_slug === 'mcp') {
      ...
      return {
        title: pkg?.display_name || mcp.title,
    ...
    return {
      title: pkg?.display_name || c.subclass_slug || c.kind,
    ...
    <div className="flex flex-col gap-5">
      {view === 'library' ? (
        ...
        <AsyncBoundary
          ...
          emptyFallback={
            <EmptyState
              icon={<Cable size={24} className="text-[var(--text-subtle)]" />}
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-start gap-3">
                    <ConnectorBrandIcon icon={meta.icon} label={meta.title} />
                  ...
                  {meta.isMcp ? (
                    ...
                    <>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <StatusPill state={c.conflict_policy ? 'ok' : 'idle'}>
                          {c.conflict_policy || 'no conflict rule'}
                        </StatusPill>
                      </div>
                      <Text variant="body-sm" tone="subtle" as="p" className="mt-1">
                  ...
                  {showConfig && c.subclass_slug === 'gmail' ? (
                    <div className="mt-3">
                      <GmailConnectorSetup
                  ...
                  {showConfig && c.subclass_slug === 'quickbooks' ? (
                    <div className="mt-3">
                  ...
                  {c.subclass_slug === 'mcp' ? (
                    <>
                      <Button
                        ...
                        disabled={refreshMcpMut.isPending && refreshMcpMut.variables === c.id}
                      ...
                      <Button
                        ...
                        disabled={healthMut.isPending && healthMut.variables === c.id}
                      ...
                      <Button
                        ...
                        disabled={
                          reauthMut.isPending && reauthMut.variables === c.id
                        }
                    ...
                    <Button
                      ...
                      disabled={syncMut.isPending && syncMut.variables === c.id}
                      aria-label={`Sync connector ${c.id}`}
                    >
                      Sync
                    </Button>
                  )}
                  {(c.subclass_slug === 'gmail' || c.subclass_slug === 'quickbooks') ? (
                    <Button
                      type="button"
                      variant="ghost"
                      size="xs"
                      ...
                      onClick={() =>
                        setConfiguringId(prev => (prev === c.id ? null : c.id))
                      }
