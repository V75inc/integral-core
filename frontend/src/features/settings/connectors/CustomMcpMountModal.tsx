...
export function CustomMcpMountModal({ open, onClose, onMounted }: Props) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (data.ok && data.connector) {
        toast.showToast('Custom MCP server connected', 'success');
        reset();
  ...
  return (
    ...
    >
      ...
      <Field label="Display name" hint="Optional label for this server.">
        <Input
          value={displayName}
      ...
      {transport === 'stdio' ? (
        ...
        <>
          ...
          <Field
            ...
            hint="Optional. TickTick: paste the API token from Settings → Account → API Token (Bearer is added if missing). Leave blank to sign in with OAuth."
