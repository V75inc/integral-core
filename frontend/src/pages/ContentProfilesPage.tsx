...
import { contentProfilesApi } from '../api';
...
function ContentProfileCard({ cp }: { cp: ContentProfileNode }) {
...
export function ContentProfilesPage() {
  ...
  usePublishPageContext({
    pageKind: 'content_profiles_list',
    metadata: {
  ...
  return (
    <PageShell>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map(cp => (
            <ContentProfileCard key={cp.id} cp={cp} />
          ))}
        </div>
      )}
      </PageSection>
